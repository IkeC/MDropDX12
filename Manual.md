...
Press **F8** or **Ctrl+L** to open the Settings window. It provides a 10-tab interface with dark theme support.
...
Adjust the overall color of the visualizer output via Settings > Colors tab. The hue shift rotates all colors around the color wheel. These can be controlled live via the Milkwave Remote IPC protocol.
...
These all default to 0 and can be changed in Settings > Colors tab or via the Remote. The values are used in the `shiftHSV()` function which is included in MDropDX12's `include.fx` file.
