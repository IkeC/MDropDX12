...
class ConfigBackend {
 public:
  ...
  // Every key in one section. Added for the migrations that move a whole
  // section out of settings.ini: copying from a hand-written list of key names
  // would silently drop anything the list forgot, and the section is deleted
  // straight afterwards, so "forgot" would mean "lost".
  virtual std::vector<std::wstring> KeyNames(const wchar_t* section) const = 0;
