...
)" R"(// MD3 PRO's FFT texel domain lives in the PRESETS now, not here (#141).
//
// md3_fft_bin / md3_fft / md3_fft_tex, and the MD3_FFT_REL_GATE and
// MD3_FFT_TAPS that tune them, were defined in this preamble and were reached
// by five recovered presets. They now travel inside those presets' own shader
// text, for two reasons.
//
// A recovered preset is SELF-CONTAINED. The treatment is a property of shaders
// recovered from MD3, not of this engine, so a preset that needs it carrying it
// is what makes it independent of the engine version -- and it is already
// independent of the audio profile, since the profile's own copy of the
// treatment was turned off in issue 139.
//
// And the curve is then editable. It has been tuned by measurement twice and
// got it wrong once; every further experiment used to be an edit-compile-run
// cycle and is now a text edit and a reload.
//
// _fftParams.z -- the frame's largest bin -- is still published here every
// frame whether or not the audio profile normalises, because that is engine
// state and the presets read it.
//
// If a preset ever needs these back, note that the parameter must NOT be named
// `uv`: it is a macro below this point (#define uv _uv.xy), so `float2 uv` in
// preset text expands to `float2 _uv.xy` and the compiler reports
// "unexpected token '.'". That is what stopped the first attempt at this.
