...
public:
  ...
  ~AudioClientStopOnExit() {
    ...
    if (FAILED(hr)) {
      wchar_t buf[128];
      FormatTo(buf, L"IAudioClient::Stop failed: hr = 0x%08x\n", hr);
...
public:
  ...
  ~AvRevertMmThreadCharacteristicsOnExit() {
    if (!AvRevertMmThreadCharacteristics(m_hTask)) {
      wchar_t buf[128];
      FormatTo(buf, L"AvRevertMmThreadCharacteristics failed: last error is %d\n", GetLastError());
      OutputDebugStringW(buf);
    }
  }
private:
...
public:
  ...
  ~CancelWaitableTimerOnExit() {
    if (!CancelWaitableTimer(m_h)) {
      wchar_t buf[128];
      FormatTo(buf, L"CancelWaitableTimer failed: last error is %d\n", GetLastError());
      OutputDebugStringW(buf);
    }
  }
private:
...
public:
  ...
  ~CloseHandleOnExit() {
    if (!CloseHandle(m_h)) {
      wchar_t buf[128];
      FormatTo(buf, L"CloseHandle failed: last error is %d\n", GetLastError());
      OutputDebugStringW(buf);
    }
  }
private:
...
public:
  ...
  ~PropVariantClearOnExit() {
    ...
    if (FAILED(hr)) {
      wchar_t buf[128];
      FormatTo(buf, L"PropVariantClear failed: hr = 0x%08x\n", hr);
      OutputDebugStringW(buf);
    }
  }
private:
...
public:
  ...
  ~SetEventOnExit() {
    if (!SetEvent(m_h)) {
      wchar_t buf[128];
      FormatTo(buf, L"SetEvent failed: last error is %d\n", GetLastError());
      OutputDebugStringW(buf);
    }
  }
private:
...
public:
  ...
  ~WaitForSingleObjectOnExit() {
    ...
    if (WAIT_OBJECT_0 != dwWaitResult) {
      wchar_t buf[128];
      FormatTo(buf, L"WaitForSingleObject returned unexpected result 0x%08x, last error is %d\n", dwWaitResult, GetLastError());
      OutputDebugStringW(buf);
    }
  }
private:
