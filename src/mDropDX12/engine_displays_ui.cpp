...
void DisplaysWindow::BuildOutputsPage(int x, int y, int rw, int lineH, int gap) {
  ...
  // Buttons: Add Spout, Remove, Refresh
  {
    ...
    }
    {
      ...
      HWND hOpacityEdit = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"100",
      ...
      if (hOpacityEdit && hFont) SendMessage(hOpacityEdit, WM_SETFONT, (WPARAM)hFont, TRUE);
      PAGE_TC(0, hOpacityEdit);
      ...
      if (hSpin) {
        SendMessage(hSpin, UDM_SETBUDDY, (WPARAM)hOpacityEdit, 0);
  ...
  y += lineH + gap;
  PAGE_TC(0, CreateCheck(hw,
    L"Independent render (per-orientation re-draw; same-orient reuses primary when larger)",
    IDC_MW_DISP_INDEPENDENT, x, y, rw, lineH, hFont, p->m_bMirrorIndependentDefault));
  y += lineH + gap;

  // Independent mirror FPS (indented under the Independent render checkbox).
  // "Match primary" steps the mirror sim once per primary frame — the mirror
  // integrates its feedback per sim frame, so an equal rate is what makes its
  // motion run at the same SPEED as the window (2026-08-23). A cap above the
  // primary's rate makes the mirrors move faster, not smoother: the render
  // thread owns the panel blits, so what a panel SHOWS can never exceed the
  // primary's fps no matter how fast the worker simulates.
  {
    int ind = MulDiv(20, lineH, 26);
    int lblW = MulDiv(60, lineH, 26);
    int cbW = MulDiv(150, lineH, 26);
    PAGE_TC(0, CreateLabel(hw, L"FPS:", x + ind, y, lblW, lineH, hFont));
    HWND hFps = CreateWindowExW(0, L"COMBOBOX", NULL,
      WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_TABSTOP,
      x + ind + lblW + 4, y, cbW, lineH * 6, hw,
      (HMENU)(INT_PTR)IDC_MW_DISP_MAXFPS, GetModuleHandle(NULL), NULL);
    if (hFps && hFont) SendMessage(hFps, WM_SETFONT, (WPARAM)hFont, TRUE);
  ...
  // Same flag as Settings → System → "Disable HUD on mirrors"; surfaced here
  // so it is findable next to the other mirror options.
  {
    int ind = MulDiv(20, lineH, 26);
    PAGE_TC(0, CreateCheck(hw, L"Hide HUD on mirrors",
      IDC_MW_DISP_HUD_DISABLE, x + ind, y, rw - ind, lineH, hFont,
      p->m_bDisableMirrorHud));
  }
  y += lineH + gap + 4;
...
void DisplaysWindow::BuildVideoInputPage(int x, int y, int rw, int lineH, int gap) {
...

  {
    ...
    HWND hSenderCombo = CreateWindowExW(0, L"COMBOBOX", NULL,
      WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL | WS_TABSTOP,
      x + sLbl + 4, y, rw - sLbl - 4 - refreshW - 8, lineH * 8, hw,
      (HMENU)(INT_PTR)IDC_MW_SPINPUT_SENDER, GetModuleHandle(NULL), NULL);
    if (hSenderCombo && hFont) SendMessage(hSenderCombo, WM_SETFONT, (WPARAM)hFont, TRUE);
  ...
  if (code == BN_CLICKED) {
    ...
    case IDC_MW_DISP_FULLSCREEN: {
      ...
      if (isPrimary) {
        ...
        // wParam 1 = ensure enter, 0 = ensure exit (see App.cpp WM_MW_FULLSCREEN).
        if (hw)
          PostMessage(hw, WM_MW_FULLSCREEN, bChecked ? 1 : 0, 0);
      } else {
