...
bool PipeNameBaseIsValid(const wchar_t* base) {
    ...
    size_t n = 0;
    for (const wchar_t* p = base; *p; ++p, ++n) {
        const bool ok = (*p >= L'A' && *p <= L'Z') || (*p >= L'a' && *p <= L'z')
                     || (*p >= L'0' && *p <= L'9') || *p == L'_';
        if (!ok) return false;
    }
    return n > 0 && n < 40;
...
void PipeServer::Start(HWND hTargetWindow, UINT wmIPCMessage, UINT wmSignalBase,
    ...
    m_dwOnlyFromPid = dwOnlyFromPid;

    // Build pipe name: \\.\pipe\Milkwave_<PID>
    // <base>_<pid>, where the base is whatever Rename last set or the default.
    // The PID is always appended -- see Rename.
    const wchar_t* nameBase = m_szNameBase[0] ? m_szNameBase : L"Milkwave_";
    FormatTo(m_szPipeName, L"\\\\.\\pipe\\%s%u", nameBase,
             GetCurrentProcessId());
...
bool PipeServer::SetNameBase(const wchar_t* base)
{
    if (!PipeNameBaseIsValid(base)) return false;
    // The underscore lives here rather than in the caller's string, so every
    // name has the same shape whatever was asked for.
    FormatTo(m_szNameBase, L"%s_", base);
    return true;
}

bool PipeServer::Rename(const wchar_t* base)
{
    // Validated BEFORE anything is torn down. A rejected name must change
    // nothing at all, and a server that returns false here is still serving.
    if (!PipeNameBaseIsValid(base)) return false;

    const HWND  hWnd     = m_hTargetWindow;
    const UINT  wmIPC    = m_wmIPCMessage;
    const UINT  wmSig    = m_wmSignalBase;
    const DWORD onlyFrom = m_dwOnlyFromPid;

    Stop();
    SetNameBase(base);
    Start(hWnd, wmIPC, wmSig, onlyFrom);
    return true;
}
...
void PipeServer::Send(const wchar_t* message) {
    ...
    // Outside the lock, deliberately -- see SetOutboundHook. This is what
    // carries every reply to a TCP client; without it a caller over TCP can
    // send commands and read almost nothing back (forgejo#33).
    if (m_outboundHook)
        m_outboundHook(message);
}
...
void PipeServer::ServerLoop() {
    ...
    while (!m_bShutdown.load()) {
        ...
        // A child answers to its parent and to nobody else.
        //
        // The mDxChild_ name keeps children out of everything that goes
        // enumerating for a visualizer to drive, but a name is a convention,
        // not a control: anyone who guesses it still connects. This is the
        // control. A child holds a reduced state that only the parent knows how
        // to keep coherent -- its preset directory, cycle timer and placement
        // are all replayed from the parent's DisplayOutputConfig on every spawn
        // -- so a stray client driving it directly produces changes that
        // silently revert, which is worse than being refused outright.
        if (m_dwOnlyFromPid != 0) {
            ULONG clientPid = 0;
            if (!GetNamedPipeClientProcessId(hConnected, &clientPid) ||
                clientPid != m_dwOnlyFromPid) {
                PipeLog("PipeServer: refused client pid %lu (only %lu may drive "
                        "this instance)\n",
                        (unsigned long)clientPid,
                        (unsigned long)m_dwOnlyFromPid);
        ...
        ctx->pServer = this;
        ctx->bStrict = m_bStrictDefault;   // -child and -test start strict
        ...
        if (GetNamedPipeClientProcessId(hConnected, &clientPid) && clientPid != 0) {
            ...
            if (hProc) {
                ...
                if (QueryFullProcessImageNameW(hProc, 0, ctx->szClientExePath, &pathLen)) {
                    ...
                    CopyTo(m_szLastClientExePath, ctx->szClientExePath);
...
void PipeServer::ClientHandler(PipeClientContext* ctx) {
    ...
    auto absorb = [&](DWORD bytesRead, bool more) {
        ...
        if (!overlong && !assembled.empty()) {
            ...
            DispatchMessage(assembled.data(), assembled.size() - 1, ctx);
        }
        assembled.clear();
        overlong = false;
    };
...
void PipeServer::DispatchMessage(const wchar_t* message, size_t len,
    ...
    // STRICT is answered HERE, and never reaches the engine (issue 102).
    //
    // It is a property of this connection, so the connection layer is the only
    // place that can honour it: by the time a message has been posted to the
    // render window the client it came from is no longer identifiable, and a
    // shared "current client" would race two clients against each other.
    //
    // Handling it here also means it costs the engine nothing -- no keyword in
    // the dispatcher, no entry in the vocabulary the fallback has to route
    // around, and no way for a STRICT sent by one client to affect another.
    if (ctx && wcsncmp(message, L"STRICT", 6) == 0 &&
        (message[6] == L'\0' || message[6] == L'=')) {
        if (message[6] == L'=') {
            // The spellings someone would actually write, and an UNRECOGNISED
            // one is an ERROR rather than "off".
            //
            // That is not fussiness. _wtoi("ON") is 0, so the obvious parse
            // reads STRICT=ON as a request to turn strictness OFF -- silently
            // disabling the feature whose entire purpose is to stop a mistyped
            // command from passing unnoticed. The same for TRUE and YES.
            const wchar_t* v = message + 7;
            int want = -1;
            if (!_wcsicmp(v, L"1") || !_wcsicmp(v, L"ON") ||
                !_wcsicmp(v, L"TRUE") || !_wcsicmp(v, L"YES"))
                want = 1;
            else if (!_wcsicmp(v, L"0") || !_wcsicmp(v, L"OFF") ||
                     !_wcsicmp(v, L"FALSE") || !_wcsicmp(v, L"NO"))
                want = 0;
            if (want < 0) {
                wchar_t err[160];
                swprintf_s(err, L"ERR|STRICT|%.80s|expected 1/0, on/off, true/false, yes/no", v);
                Send(err);
                return;
            }
            // Sticky for the life of THIS connection: once a client turns it
            // off it stays off until that same client turns it back on, and a
            // new connection starts again at the instance default.
            ctx->bStrict = (want != 0);
        }
        wchar_t reply[32];
        swprintf_s(reply, L"STRICT=%d", ctx->bStrict ? 1 : 0);
        Send(reply);
        PipeLog("PipeServer: client %d strict=%d\n", ctx->nClientId, ctx->bStrict ? 1 : 0);
        return;
    }
    ...
    if (wcsncmp(message, L"SPOUT_SENDER=", 13) == 0) {
        ...
        if (copy) {
            wcsncpy_s(copy, nameLen + 1, name, _TRUNCATE);
    ...
    if (copy) {
        wcsncpy_s(copy, msgLen + 1, message, _TRUNCATE);
        // wParam 1 = ordinary, 3 = strict. The free field, rather than a member
        // the window thread would read after the fact: a post is asynchronous,
        // so anything shared would be read for the WRONG message as soon as two
        // clients overlapped. (2 is taken -- App.cpp posts it with lParam 0.)
        const WPARAM wp = (ctx && ctx->bStrict) ? (WPARAM)3 : (WPARAM)1;
        if (!PostMessageW(m_hTargetWindow, m_wmIPCMessage, wp, (LPARAM)copy)) {
...
static const SignalEntry s_signalTable[] = {
    ...
    { L"SHOW_COVER",           110, false, 0 },
    { L"FULLSCREEN",           160, false, MW_FS_TOGGLE },
    { L"WATERMARK",            161, false, 0 },
    // MW_FS_TOGGLE, not 0: 0 is MW_FS_EXIT, and a signal stuck at it is exactly
    // how SIGNAL|FULLSCREEN became a one-way exit (forgejo#23).
    { L"BORDERLESS_FS",        162, false, MW_FS_TOGGLE },
    { L"STRETCH",              163, false, 0 },
    { L"MIRROR",               164, false, 0 },
    { L"MIRROR_WM",            165, false, 0 },
    { L"MIRROR_INDEPENDENT",   167, false, 0 },
    { L"ALWAYS_ON_TOP",        168, false, 0 },
...
bool PipeSendToExistingInstance(const wchar_t* message) {
    ...
    if (Process32FirstW(hSnap, &pe)) {
        do {
            ...
            wchar_t pipeName[64];
            // Milkwave_ only, which is what skips CHILDREN. They run the same
            // exe, so the image-path test above passes for them, and a
            // double-clicked .milk could be forwarded to a child -- which
            // would load it and then be stomped by the parent's next
            // reconfigure. A child serves mDxChild_<PID>, so this simply
