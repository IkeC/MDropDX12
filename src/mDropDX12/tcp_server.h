...
struct TcpClientConnection {
    ...
    ULONGLONG connectedAt = 0;   // for "dropped after Ns" in the log // GetTickCount64
...
private:
    ...
    // reason is logged, so a drop is attributable rather than merely counted.
    void RemoveClient(size_t index, const char* reason);
