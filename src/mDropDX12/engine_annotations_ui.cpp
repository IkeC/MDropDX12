...
#include "preset_replacements.h"  // PresetReplacements(), for the replacement slot
#include "utility.h"
#include "format_to.h"
#include <CommCtrl.h>
#include <Uxtheme.h>
...
static const wchar_t* FlagsToDisplay(uint32_t flags) {
    ...
    buf[0] = 0;
    if (flags & PFLAG_FAVORITE) AppendTo(buf, L"\x2605 ");
    if (flags & PFLAG_ERROR)    AppendTo(buf, L"\x26A0 ");
    if (flags & PFLAG_SKIP)     AppendTo(buf, L"\x2298 ");
    if (flags & PFLAG_BROKEN)   AppendTo(buf, L"\x2716 ");
    return buf;
}
...
static std::wstring FormatDuration(int seconds) {
    ...
    wchar_t buf[32];
    if (seconds < 60)            FormatTo(buf, L"%ds", seconds);
    else if (seconds < 3600)     FormatTo(buf, L"%dm %02ds", seconds / 60, seconds % 60);
    else                         FormatTo(buf, L"%dh %02dm", seconds / 3600, (seconds % 3600) / 60);
    return buf;
}
...
void AnnotationsWindow::RefreshList() {
    ...
    //
    // COPIES, not pointers, and the guard is dropped before a single row is
    // drawn (#11). This loop used to keep `const PresetAnnotation*` into the
    // map across the sort AND across every LV_InsertItemW below -- and
    // LV_InsertItemW is a SendMessage, so the list-fill pumps. A reload on the
    // IPC thread arriving anywhere in there freed every pointer in `rows`.
    // That is the exact crash test_annotations_thread_safety.py reproduces.
    //
    // The copy costs one PresetAnnotation per VISIBLE row, after filtering,
    // which is what the window was about to render anyway.
    std::vector<PresetAnnotation> rows;
    p->m_annot.with([&](Engine::AnnotStore& store) {
    for (auto& [key, a] : store.byKey) {
    ...
    std::stable_sort(rows.begin(), rows.end(),
        [&](const PresetAnnotation& x, const PresetAnnotation& y) {
            const PresetAnnotation* xa = &x;
            const PresetAnnotation* ya = &y;
            int cmp = 0;
            switch (sortCol) {
    ...
    int row = 0;
    for (const PresetAnnotation& a : rows) {
        ...
        if (nCopies > 0) FormatTo(copiesBuf, L"%d", nCopies);
        ...
        wchar_t playsBuf[16] = {};
        if (a.useCount > 0) FormatTo(playsBuf, L"%d", a.useCount);
        ...
        if (!a.notes.empty()) {
            ...
            if (a.notes.size() > 76) AppendTo(notesPreview, L"...");
        }
        ...
        if (p->IsAnnotationKnownMissing(a)) {
            wchar_t gone[96];
            FormatTo(gone, L"(file missing) %.60s", notesPreview);
...
// data the same way.
// Copy an annotation out from under the guard, or answer false (#11).
//
// Every site in this file goes on to build a menu, open a modal dialog or put
// up a MessageBox, and all three pump -- so none of them may run inside a
// with(), and none of them may keep the PresetAnnotation* they used to. A
// whole-entry copy is the cheap answer here: these fire on a right-click or a
// selection change, not per frame.
static bool CopyAnnotation(Engine* p, const wchar_t* fn, PresetAnnotation& out) {
    return p->WithAnnotation(fn, /*create=*/false,
                             [&](PresetAnnotation& a) { out = a; });
}
...
void AnnotationsWindow::ShowRowContextMenu(int screenX, int screenY) {
    ...
    PresetAnnotation a;
    const bool have = CopyAnnotation(p, fn.c_str(), a);
    const bool isFav    = have && (a.flags & PFLAG_FAVORITE);
    const bool isSkip   = have && (a.flags & PFLAG_SKIP);
    const bool isBroken = have && (a.flags & PFLAG_BROKEN);
    const bool hasError = have && (a.flags & PFLAG_ERROR);
    ...
    const int curRating = have ? Engine::AverageRating(a) : 0;
    for (int r = 5; r >= 0; r--) {
        wchar_t label[32];
        if (r == 0) CopyTo(label, L"Unrated");
        else { label[0] = 0; for (int t = 0; t < r; t++) AppendTo(label, L"&05"); }
    ...
    using mdrop::Engine;
    const int curCanvas = (have && a.hasCanvasMax) ? a.canvasMax : 0;
    ...
    for (int i = 0; i < (int)_countof(Engine::kCanvasChoices); i++) {
        ...
        AppendMenuW(hCanvas, MF_STRING | (curCanvas == Engine::kCanvasChoices[i] ? MF_CHECKED : 0),
    ...
    const float curDamp = (have && a.hasFeedbackDamp) ? a.feedbackDamp : 0.0f;
    ...
    AppendMenuW(hMenu, MF_SEPARATOR, 0, NULL);
    {
        // Replacement preset (issue 120). A preset carrying MD31=/MD32= ships a
        // stub shader and renders black, because MD3 PRO would have supplied the
        // real one from its own cache; designating a stand-in is how it renders
        // at all. It is offered on EVERY row, not just keyed ones, because
        // substituting your own edit of a preset is the same operation.
        const std::wstring cur = have && a.hasReplacementPreset ? a.replacementPreset
                                                                : std::wstring();
        HMENU hRepl = CreatePopupMenu();
        AppendMenuW(hRepl, MF_STRING, IDC_MW_ANNOT_REPL_SET,
                    L"Designate a file...");
        AppendMenuW(hRepl, MF_STRING, IDC_MW_ANNOT_REPL_COPY,
                    L"Copy this preset to a replacement...");
        AppendMenuW(hRepl, MF_STRING | (cur.empty() ? MF_GRAYED : 0),
                    IDC_MW_ANNOT_REPL_EDIT, L"Edit the replacement");
        AppendMenuW(hRepl, MF_SEPARATOR, 0, NULL);
        AppendMenuW(hRepl, MF_STRING | (cur.empty() ? MF_GRAYED : 0),
                    IDC_MW_ANNOT_REPL_CLEAR, L"Clear designation");
        AppendMenuW(hMenu, MF_POPUP, (UINT_PTR)hRepl,
                    cur.empty() ? L"Replacement Preset"
                                : L"Replacement Preset  (set)");
    }
    AppendMenuW(hMenu, MF_SEPARATOR, 0, NULL);
    ...
    void DoBuildControls(int clientW, int clientH) override {
        ...
        // ── Usage ──
        {
            ...
            if (m_annot.useCount > 0) {
                ...
                FormatTo(plays, L"%d  (%.40s on screen)", m_annot.useCount,
            ...
            m_pEngine ? KnownCopiesOf(m_pEngine, m_annot) : std::vector<std::wstring>();
        {
            ...
            if (scanned >= 2)
                FormatTo(copiesStr, L"%d files share this content", scanned);
            else if (scanned == 1)
                CopyTo(copiesStr, L"1 - no other copy found by the last scan");
            else if (copies.size() > 1)
                FormatTo(copiesStr, L"%d known location%s (run Find Copies for a full count)",
                         (int)copies.size(), copies.size() == 1 ? L"" : L"s");
            else
                CopyTo(copiesStr, L"not scanned - use Find Copies");
        ...
        // a preset's whole configuration can be read at once.
        {
            ...
            // The whole path, not a leaf: a designation that has gone stale is
            // read here, and "which of these three copies is it" is the
            // question being asked when it has.
            if (m_annot.hasReplacementPreset) {
                std::wstring rep = m_annot.replacementPreset;
                if (!rep.empty() &&
                    GetFileAttributesW(rep.c_str()) == INVALID_FILE_ATTRIBUTES)
                    rep += L"   (MISSING)";
                row(L"Runs:", rep.empty() ? L"(explicitly none)" : rep.c_str());
            }

            std::wstring canvas = L"(inherit from tags)";
            if (m_annot.hasCanvasMax) {
                wchar_t b[32];
                FormatTo(b, L"%d px", m_annot.canvasMax);
                canvas = b;
            }
            row(L"Canvas:", canvas);
...
void AnnotationsWindow::ShowDetailsDialog() {
    ...
    if (fn.empty()) return;
    PresetAnnotation a;
    if (!CopyAnnotation(m_pEngine, fn.c_str(), a)) return;
    // The dialog is modal and runs its own message pump, so it is handed a
    // COPY -- it must not hold a reference into the store while it is open.
...
class ImportAnnotationsDialog : public ModalDialog {
    ...
    void DoBuildControls(int clientW, int clientH) override {
        ...
        for (auto& [key, imp] : m_imported) {
            ...
            // Current annotation (if any). Copied out: LV_SetItemTextW is a
            // SendMessage, so this loop pumps.
            PresetAnnotation cur;
            const bool haveCur = CopyAnnotation(m_pEng, imp.filename.c_str(), cur);
            LV_SetItemTextW(m_hList, row, 2, (LPWSTR)RatingToStars(haveCur ? cur.rating : 0));
            LV_SetItemTextW(m_hList, row, 3, (LPWSTR)FlagsToDisplay(imp.flags));
            LV_SetItemTextW(m_hList, row, 4, (LPWSTR)FlagsToDisplay(haveCur ? cur.flags : 0));

            // Check by default if imported has data and current doesn't
            bool shouldCheck = (!haveCur || cur.rating == 0) && (imp.rating > 0 || imp.flags != 0);
    ...
    void ImportOne(const PresetAnnotation& imp, bool merge) {
        // Read-modify-write, so it mutates inside the guard rather than
        // copying out -- there is nothing here that pumps.
        const bool ok = m_pEng->WithAnnotation(imp.filename.c_str(), /*create=*/true,
                                               [&](PresetAnnotation& e) {
        PresetAnnotation* cur = &e;
        if (merge) {
            // Take higher rating
    ...
    LRESULT DoCommand(int id, int code, LPARAM lParam) override {
        ...
        if (id == IDC_MW_IMPORT_ALL) {
            for (auto& [key, imp] : m_imported)
                ImportOne(imp, false);
            m_pEng->m_annot.with([](Engine::AnnotStore& s) { s.dirty = true; });
...
static std::wstring FormatFileTime(const FILETIME& ft) {
    ...
    wchar_t buf[32];
    FormatTo(buf, L"%04d-%02d-%02d %02d:%02d",
...
static std::wstring FormatBytes(uint64_t n) {
    wchar_t buf[32];
    if (n < 1024)            FormatTo(buf, L"%llu B", (unsigned long long)n);
    else if (n < 1024 * 1024) FormatTo(buf, L"%.1f KB", n / 1024.0);
    else                      FormatTo(buf, L"%.1f MB", n / (1024.0 * 1024.0));
    return buf;
}
    ...
    LRESULT DoMessage(UINT msg, WPARAM wParam, LPARAM lParam) override {
        if (msg == WM_TIMER && wParam == IDT_ANNOT_DUPESCAN) {
            ...
            if (m_hStatus) {
                ...
                wchar_t buf[160];
                if (m_cancel.load())
                    FormatTo(buf, L"Stopping...");
                else if (total <= 0)
                    FormatTo(buf, L"Looking for presets... %d found", done);
                else
                    FormatTo(buf, L"Reading presets... %d of %d", done, total);
...
class DuplicatesReportDialog : public ModalDialog {
    ...
    void Populate() {
        ...
        for (size_t i = 0; i < m_rows.size(); i++) {
            ...
            wchar_t num[16];
            FormatTo(num, L"%d", (int)m_rows[i].group + 1);
        ...
        if (m_hSummary) {
            wchar_t buf[512];
            if (m_groups.empty()) {
                CopyTo(buf, L"No duplicate presets found.");
            } else {
    ...
    std::wstring BuildReportText() const {
        ...
        for (size_t g = 0; g < m_groups.size(); g++) {
            wchar_t head[128];
            FormatTo(head, L"[%d] %d copies  (hash %.20s)\r\n",
...
void AnnotationsWindow::DoBuildControls() {
    ...
    // function's job is now only to create the controls and fill them in.
    {
        ...
        m_hFilterCombo = CreateCombo(hw, IDC_MW_ANNOTWIN_FILTER, G.cbFilter.left,
          G.cbFilter.top, G.cbFilter.right - G.cbFilter.left, G.cbFilter.bottom - G.cbFilter.top, hFont);
    ...
    // RefreshOverrideCombos.
    {
        ...
        for (int c = 0; c < (int)_countof(cols); c++) {
            ...
            *cols[c].combo = CreateCombo(hw, cols[c].id, rc2.left, rc2.top,
                                         rc2.right - rc2.left, rc2.bottom - rc2.top, hFont);
        }
...
void AnnotationsWindow::RefreshOverrideCombos() {
    ...
    const std::wstring fn = GetSelectedFilename();
    // One copy feeds every combo below, so they cannot disagree with each
    // other -- and each SendMessage that fills one would otherwise pump with
    // the entry still held.
    PresetAnnotation a;
    const bool have = !fn.empty() && CopyAnnotation(m_pEngine, fn.c_str(), a);
    ...
    if (m_hCanvasCombo) {
        ...
        const int cur = (have && a.hasCanvasMax) ? a.canvasMax : 0;
        int idx = 0;
        for (int i = 0; i < (int)_countof(kChoices); i++)
            if (kChoices[i] == cur) { idx = i; break; }
        SendMessage(m_hCanvasCombo, CB_SETCURSEL, idx, 0);
        EnableWindow(m_hCanvasCombo, !fn.empty());
    }
    if (m_hDampCombo) {
        const float cur = (have && a.hasFeedbackDamp) ? a.feedbackDamp : 0.0f;
        SendMessage(m_hDampCombo, CB_SETCURSEL, Engine::DampChoiceIndex(cur), 0);
        EnableWindow(m_hDampCombo, !fn.empty());
    }
    // These two were the only members of this group never enabled or disabled:
    // the three slots below do it in their loop, and these had only their
    // selection set. With no row selected they stayed live, showing the first
    // choice, and their handlers -- which both require a filename -- quietly
    // did nothing when used.
    //
    // The condition is !fn.empty() rather than the slots' `a != nullptr`, and
    // deliberately so: SetPresetCanvasMaxByFile and SetPresetFeedbackDampByFile
    // CREATE the annotation, so a preset that has none yet is a perfectly good
    // target. Disabling on `a` would have made a canvas limit unsettable on
    // exactly the presets most likely to need one.
    ...
    Slot slots[3] = {
        { m_hShaderCombo, &m_shaderNames,
          have && a.hasShaderOverride, have ? a.shaderOverride : std::wstring() },
        { m_hVfxCombo, &m_vfxProfNames,
          have && a.hasVfxProfile, have ? a.vfxProfile : std::wstring() },
        { m_hAudioCombo, &m_audioProfNames,
          have && a.hasAudioProfile, have ? a.audioProfile : std::wstring() },
    };
...
LRESULT AnnotationsWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
    ...
    if (code == 0) {
        ...
        if (!cfn.empty()) {
            PresetAnnotation ca;
            const bool haveCa = CopyAnnotation(p, cfn.c_str(), ca);
            if (id == IDC_MW_ANNOT_FAV) {
                p->SetPresetFlag(cfn.c_str(), PFLAG_FAVORITE, !(haveCa && (ca.flags & PFLAG_FAVORITE)));
                RefreshList(); return 0;
            }
            if (id == IDC_MW_ANNOT_SKIP) {
                p->SetPresetFlag(cfn.c_str(), PFLAG_SKIP, !(haveCa && (ca.flags & PFLAG_SKIP)));
                RefreshList(); return 0;
            }
            if (id == IDC_MW_ANNOT_BROKEN) {
                p->SetPresetFlag(cfn.c_str(), PFLAG_BROKEN, !(haveCa && (ca.flags & PFLAG_BROKEN)));
                RefreshList(); return 0;
            }
    ...
    if (id == IDC_MW_ANNOTWIN_LOAD) {
        ...
        std::wstring fn = GetSelectedFilename();
        if (!fn.empty()) {
            // Copied out first: ResolveAnnotationPath does file I/O and the
            // MessageBoxW below is modal. Neither may run under the guard.
            PresetAnnotation a;
            const bool have = CopyAnnotation(p, fn.c_str(), a);
            const std::wstring path = have ? p->ResolveAnnotationPath(a) : std::wstring();
            if (path.empty()) {
    ...
    if (id == IDC_MW_ANNOTWIN_REMOVEMISSING) {
        ...
        // Counted under the guard; the MessageBoxW below must never run
        // inside it -- a modal dialog holds the lock for as long as the user
        // takes to read it, and every other thread waits.
        int doomed = 0;
        p->m_annot.with([&](Engine::AnnotStore& store) {
            for (const auto& kv : store.byKey)
        ...
        RefreshList();
        wchar_t msg[128];
        FormatTo(msg, L"Purged %d annotation%s whose preset file is missing.",
                 n, n == 1 ? L"" : L"s");
        MessageBoxW(m_hWnd, msg, L"Purge Missing", MB_OK | MB_ICONINFORMATION);
        return 0;
    }

    // ── Replacement preset (issue 120) ─────────────────────────────────
    //
    // These arrive from the context menu, so `code` is 0, NOT BN_CLICKED --
    // testing for BN_CLICKED here would make them silently do nothing.
    if (id == IDC_MW_ANNOT_REPL_SET || id == IDC_MW_ANNOT_REPL_COPY ||
        id == IDC_MW_ANNOT_REPL_EDIT || id == IDC_MW_ANNOT_REPL_CLEAR) {
        const std::wstring fn = GetSelectedFilename();
        if (fn.empty()) return 0;
        PresetAnnotation a;
        const bool have = CopyAnnotation(p, fn.c_str(), a);

        if (id == IDC_MW_ANNOT_REPL_CLEAR) {
            p->SetPresetReplacement(fn.c_str(), std::wstring(), false);
            RefreshList();
            return 0;
        }

        if (id == IDC_MW_ANNOT_REPL_EDIT) {
            const std::wstring rep = have && a.hasReplacementPreset
                                   ? a.replacementPreset : std::wstring();
            if (rep.empty()) return 0;
            // Load the REPLACEMENT itself, not the preset that points at it.
            // The Preset Editor reads and saves through m_szCurrentPresetFile,
            // so loading the original would show the original's (stub) text
            // while the screen showed the replacement's render -- and a save
            // would write into the wrong file.
            RequestLoadPreset(-1, rep.c_str(), 0.0f);
            p->OpenPresetEditorWindow();
            return 0;
        }

        // Where the selected preset actually lives -- same resolution the Load
        // action uses, never m_szPresetDir + filename.
        const std::wstring src = have ? p->ResolveAnnotationPath(a) : std::wstring();

        if (id == IDC_MW_ANNOT_REPL_SET) {
            wchar_t szFile[MAX_PATH] = {0};
            const auto& paths = PresetReplacements().SearchPaths();
            std::wstring initial = paths.empty() ? std::wstring() : paths.front();
            OPENFILENAMEW ofn = {};
            ofn.lStructSize = sizeof(ofn);
            ofn.hwndOwner = m_hWnd;
            ofn.lpstrFilter = L"Presets (*.milk;*.milk2;*.milk3)\0*.milk;*.milk2;*.milk3\0"
                              L"All Files (*.*)\0*.*\0";
            ofn.lpstrFile = szFile;
            ofn.nMaxFile = MAX_PATH;
            ofn.lpstrTitle = L"Preset to run in place of this one";
            ofn.lpstrInitialDir = initial.empty() ? NULL : initial.c_str();
            ofn.Flags = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST;
            if (!GetOpenFileNameW(&ofn)) return 0;
            if (!src.empty() && _wcsicmp(szFile, src.c_str()) == 0) {
                MessageBoxW(m_hWnd,
                    L"That is the preset itself. A preset cannot stand in for "
                    L"itself.", L"Replacement Preset", MB_OK | MB_ICONWARNING);
                return 0;
            }
            p->SetPresetReplacement(fn.c_str(), szFile, true);
            RefreshList();
            return 0;
        }

        // COPY: duplicate this preset, and designate the copy. The point is to
        // get an editable starting body -- for an MD31 preset the original is a
        // stub, so the copy is a starting point rather than a working preset,
        // and that is still better than authoring one from nothing.
        if (src.empty()) {
            MessageBoxW(m_hWnd,
                L"That preset's file cannot be found in any of its recorded "
                L"locations, so there is nothing to copy.",
                L"Replacement Preset", MB_OK | MB_ICONWARNING);
            return 0;
        }
        wchar_t szFile[MAX_PATH] = {0};
        lstrcpynW(szFile, fn.c_str(), MAX_PATH);
        const auto& paths = PresetReplacements().SearchPaths();
        std::wstring initial = paths.empty() ? std::wstring() : paths.front();
        OPENFILENAMEW ofn = {};
        ofn.lStructSize = sizeof(ofn);
        ...
        ofn.lpstrFile = szFile;
        ofn.nMaxFile = MAX_PATH;
        ofn.lpstrTitle = L"Save the replacement copy as";
        ofn.lpstrInitialDir = initial.empty() ? NULL : initial.c_str();
        ofn.Flags = OFN_OVERWRITEPROMPT | OFN_PATHMUSTEXIST;
        if (!GetSaveFileNameW(&ofn)) return 0;
        if (_wcsicmp(szFile, src.c_str()) == 0) {
            MessageBoxW(m_hWnd,
                L"That would overwrite the preset itself.",
                L"Replacement Preset", MB_OK | MB_ICONWARNING);
            return 0;
        }
        if (!CopyFileW(src.c_str(), szFile, FALSE)) {
            wchar_t msg[512];
            FormatTo(msg, L"Could not copy the preset to:\n\n%s", szFile);
            MessageBoxW(m_hWnd, msg, L"Replacement Preset", MB_OK | MB_ICONERROR);
            return 0;
        }
        p->SetPresetReplacement(fn.c_str(), szFile, true);
        RefreshList();
    ...
    if (id == IDC_MW_ANNOTWIN_REMOVE && code == BN_CLICKED) {
        std::wstring fn = GetSelectedFilename();
        if (!fn.empty()) {
            const bool removed = p->m_annot.with([&](Engine::AnnotStore& store) {
                auto it = store.byKey.find(fn);
                if (it == store.byKey.end()) return false;
                // The hash index points AT this key; leaving it behind would
                // leave a hash resolving to an entry that no longer exists.
                if (!it->second.hash.empty()) store.byHash.erase(it->second.hash);
                store.byKey.erase(it);
                store.dirty = true;
                return true;
            });
            if (removed) {
