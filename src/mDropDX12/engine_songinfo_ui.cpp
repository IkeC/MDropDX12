...
static void PopulateSongAnimCombo(HWND hAnimCombo, Engine* p) {
  if (!hAnimCombo) return;
  SendMessage(hAnimCombo, CB_RESETCONTENT, 0, 0);
  SendMessageW(hAnimCombo, CB_ADDSTRING, 0, (LPARAM)L"(Default)");
  SendMessageW(hAnimCombo, CB_ADDSTRING, 0, (LPARAM)L"(Random)");
  for (int i = 0; i < p->m_nAnimProfileCount; i++)
    SendMessageW(hAnimCombo, CB_ADDSTRING, 0, (LPARAM)p->m_AnimProfiles[i].szName);
  int sel = 0;
  if (p->m_nSongTitleAnimProfile == -2) sel = 1;
  else if (p->m_nSongTitleAnimProfile >= 0) sel = p->m_nSongTitleAnimProfile + 2;
  SendMessage(hAnimCombo, CB_SETCURSEL, sel, 0);
}

static int ComboToSongAnimIndex(HWND hAnimCombo) {
  int sel = (int)SendMessage(hAnimCombo, CB_GETCURSEL, 0, 0);
...
void SongInfoWindow::DoBuildControls() {
  ...
  {
    HWND hSourceCombo = CreateCombo(hw, IDC_MW_SONG_SOURCE, x + lw + 4, y, rw - lw - 4,
      lineH + 4 * lineH, hFont, CBS_DROPDOWNLIST, true, WS_EX_CLIENTEDGE);
    SendMessageW(hSourceCombo, CB_ADDSTRING, 0, (LPARAM)L"SMTC (Windows)");
    SendMessageW(hSourceCombo, CB_ADDSTRING, 0, (LPARAM)L"IPC (Remote)");
    SendMessageW(hSourceCombo, CB_ADDSTRING, 0, (LPARAM)L"Window Title");
    SendMessageW(hSourceCombo, CB_SETCURSEL, p->m_nTrackInfoSource, 0);
    TrackControl(hSourceCombo);
  }
  y += lineH + 2;
  ...
  // ── Window Title profile selector + Edit button (visible when source = Window Title) ──
  {
    ...
    HWND hProfileCombo = CreateCombo(hw, IDC_MW_WT_PROFILE, x + lw + 4, y, comboW,
      lineH + 10 * lineH, hFont, vis | CBS_DROPDOWNLIST, false, WS_EX_CLIENTEDGE);
  ...
  // ── Song Title Animation Profile row ──
  {
    ...
    HWND hAnimCombo = CreateCombo(hw, IDC_MW_SONG_ANIM_COMBO, x + 16 + profileLabelW, y,
      comboW, lineH * 10, hFont);
    ...
    TrackControl(CreateBtn(hw, L"Profiles...", IDC_MW_SONG_EDIT_PROFILES,
  ...
  {
    HWND hCornerCombo = CreateCombo(hw, IDC_MW_SONG_CORNER, x + lw + 4, y, rw - lw - 4,
      lineH + 5 * lineH, hFont, CBS_DROPDOWNLIST, true, WS_EX_CLIENTEDGE);
    SendMessageW(hCornerCombo, CB_ADDSTRING, 0, (LPARAM)L"Top-Left");
    SendMessageW(hCornerCombo, CB_ADDSTRING, 0, (LPARAM)L"Top-Right");
    SendMessageW(hCornerCombo, CB_ADDSTRING, 0, (LPARAM)L"Bottom-Left");
    SendMessageW(hCornerCombo, CB_ADDSTRING, 0, (LPARAM)L"Bottom-Right");
    SendMessageW(hCornerCombo, CB_SETCURSEL, p->m_SongInfoDisplayCorner - 1, 0);
    TrackControl(hCornerCombo);
  }
  y += lineH + 2;
...
LRESULT SongInfoWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
  ...
  if (id == IDC_MW_SONG_SOURCE && code == CBN_SELCHANGE) {
    ...
    if (sel >= 0 && sel <= 2) {
      ...
      Config().SetInt(L"Milkwave", L"TrackInfoSource", sel);
  ...
  if (id == IDC_MW_WT_PROFILE && code == CBN_SELCHANGE) {
    ...
    if (sel >= 0 && sel < (int)p->m_windowTitleProfiles.size()) {
      ...
      Config().SetInt(L"Milkwave", L"WindowTitleProfile", sel);
  ...
  if (id == IDC_MW_SONG_CORNER && code == CBN_SELCHANGE) {
    ...
    if (sel >= 0 && sel <= 3) {
      ...
      Config().SetInt(L"Milkwave", L"SongInfoDisplayCorner", p->m_SongInfoDisplayCorner);
    }
    return 0;
  }
  ...
  if (id == IDC_MW_SONG_DISPLAY_SEC && code == EN_CHANGE) {
    ...
    { wchar_t sb[32]; swprintf(sb, 32, L"%.1f", p->m_SongInfoDisplaySeconds);
      Config().SetString(L"Milkwave", L"SongInfoDisplaySeconds", sb); }
    return 0;
  }
  ...
  if (code == BN_CLICKED) {
    ...
    if (bIsCheckbox) {
      ...
      case IDC_MW_SONG_OVERLAY:
        ...
        Config().SetInt(L"Milkwave", L"SongInfoOverlay", bChecked);
        return 0;
      case IDC_MW_SONG_COVER:
        p->m_DisplayCover = bChecked;
        Config().SetInt(L"Milkwave", L"DisplayCover", bChecked);
        return 0;
      case IDC_MW_SONG_ALWAYS_SHOW:
        p->m_bSongInfoAlwaysShow = bChecked;
        Config().SetInt(L"Milkwave", L"SongInfoAlwaysShow", bChecked);
        if (!bChecked) {
