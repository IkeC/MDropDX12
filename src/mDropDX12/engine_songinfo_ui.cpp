...
void SongInfoWindow::DoBuildControls() {
  ...
  {
    HWND hSourceCombo = CreateWindowExW(WS_EX_CLIENTEDGE, L"COMBOBOX", NULL,
    ...
    if (hSourceCombo && hFont) SendMessage(hSourceCombo, WM_SETFONT, (WPARAM)hFont, TRUE);
  ...
  {
    HWND hCornerCombo = CreateWindowExW(WS_EX_CLIENTEDGE, L"COMBOBOX", NULL,
      WS_CHILD | WS_VISIBLE | WS_TABSTOP | CBS_DROPDOWNLIST | CBS_HASSTRINGS,
      x + lw + 4, y, rw - lw - 4, lineH + 5 * lineH, hw, (HMENU)(INT_PTR)IDC_MW_SONG_CORNER,
      GetModuleHandle(NULL), NULL);
    if (hCornerCombo && hFont) SendMessage(hCornerCombo, WM_SETFONT, (WPARAM)hFont, TRUE);
