...
void Engine::RenderInjectEffect()
{
  ...
  const bool bPassthrough = (m_pState && m_pState->CompShaderIsGetPixelPassthrough());
  ...
  const bool bOldHasCompShader =
      ...
      m_pOldState->m_szCompShadersText[0] &&
      !m_pOldState->CompShaderIsGetPixelPassthrough();
  const bool classicPost  = (bNoCompPSO || bPassthrough) && !bOldHasCompShader;
  ...
  // *m_pState->var_pf_invert reads 1 -- do not diagnose this from STATE output.
  {
    ...
    const int gate = (bNoCompPSO ? 1 : 0) | (bPassthrough ? 2 : 0);
    if (presetFxMask != s_lastMask || gate != s_lastGate) {
      ...
      DLOG_WARN("InjectFX: mask=%u (brighten=%d darken=%d solarize=%d invert=%d) "
                "gate: noCompPSO=%d passthrough=%d classicPost=%d oldHasComp=%d  preset=%ls",
                presetFxMask, (presetFxMask & 1u) ? 1 : 0, (presetFxMask & 2u) ? 1 : 0,
                (presetFxMask & 4u) ? 1 : 0, (presetFxMask & 8u) ? 1 : 0,
                bNoCompPSO ? 1 : 0, bPassthrough ? 1 : 0, classicPost ? 1 : 0,
...
void Engine::MyRenderUI(
  ...
  //                                        and it should be visible over everything else (usually an error msg)
  {
    ...
    if (m_bShowFPS) {
      ...
      // m_fOrientFps, not m_mirrorSim.fFps: the latter is written only by
      ...
      if (m_bMirrorsActive && m_hMirrorThread && m_fOrientFps > 0.5f) {
        swprintf(buf, L"%s: %4.2f ", L"mirror", m_fOrientFps);
        MyTextOut_Shadow(buf, MTO_UPPER_RIGHT);
      }
    }
    ...
    if (m_bShowDebugInfo) {
...

      {
        ...
        if (!m_bIdleTimerEnabled) {
          swprintf(buf, L"idle off");
        } else if (m_bIdleActivated) {
          swprintf(buf, L"idle active  %s%s", action, m_bIdleAutoRestore ? L"  auto" : L"");
        } else {
          ...
          swprintf(buf, L"idle %u:%02u  %s", sec / 60u, sec % 60u, action);
        }
        MyTextOut_Color(buf, MTO_UPPER_LEFT, color);
      }
      ...
      if (m_vramBudgetBytes > 0 || m_bVramExitTriggered) {
        ...
        swprintf(buf, L"vram avail %.1f%%  usage %.0f / budget %.0f MB%s",
    ...
    if (m_waitstring.bActive) {
      ...
      if (bIsWarp || bIsComp) {
        ...
        *upper_left_corner_y += h * 2 / 3;
        ...
        wasabiApiLangString(shader_msg_id, lockMsg, 256);
      ...
      if (m_pState->m_nWarpPSVersion >= m_nMaxPSVersion &&
        ...
        swprintf(psVerMsg, wasabiApiLangString(IDS_PRESET_USES_HIGHEST_PIXEL_SHADER_VERSION), m_nMaxPSVersion);
      ...
      *upper_left_corner_y = rect.top;
      ...
      if (m_nPresets - m_nDirs == 0) {
        if (wcscmp(m_pState->m_szDesc, INVALID_PRESET_DESC) == 0) {
          wchar_t errMsg[1024];
          swprintf(errMsg, wasabiApiLangString(IDS_ERROR_NO_PRESET_FILE_FOUND_IN_X_MILK), m_szPresetDir);
        ...
        *upper_left_corner_y += PLAYLIST_INNER_MARGIN;
        ...
        if (m_bShowMenuToolTips) {
          wchar_t pageMsg[256];
          swprintf(pageMsg, wasabiApiLangString(IDS_PAGE_X_OF_X), m_nMashPreset[m_nMashSlot] / lines_available + 1, (m_nPresets + lines_available - 1) / lines_available);
        ...
        for (int pass = 0; pass < 2; pass++) {
          ...
          for (int mash = 0; mash < MASH_SLOTS; mash++) {
            ...
            // SPOUT
                        // swprintf(slotLine, L"%s%s", wasabiApiLangString(mashNames[mash]), m_presets[idx].szFilename);
            swprintf(slotLine, L"%s%s", wasabiApiLangString(mashNames[mash]), m_presets[idx].szFilename.c_str());
            RECT r2 = orig_rect;
            r2.top += stackH;
      ...
      if (m_nPresets - m_nDirs == 0) {
        ...
        swprintf(dirLine, wasabiApiLangString(IDS_DIRECTORY_OF_X), m_szPresetDir);
        ...
        *upper_left_corner_y += h / 2;
        ...
        if (m_bShowMenuToolTips) {
          wchar_t pageMsg[256];
          swprintf(pageMsg, wasabiApiLangString(IDS_PAGE_X_OF_X), m_nPresetListCurPos / lines_available + 1, (m_nPresets + lines_available - 1) / lines_available);
          DrawTooltip(pageMsg, xR, *lower_right_corner_y);
        }
      ...
      for (int i = 0; i < N; i++) {
        if (t >= m_errors[i].birthTime && t < m_errors[i].expireTime) {
          if (m_errors[i].category == ERR_MSG_BOTTOM_EXTRA_1 || m_errors[i].category == ERR_MSG_BOTTOM_EXTRA_2 || m_errors[i].category == ERR_MSG_BOTTOM_EXTRA_3) {
            ...
            bool hideForRemote = m_errors[i].bSentToRemote && m_HideNotificationsWhenRemoteActive;
