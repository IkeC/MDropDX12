...
void PresetsWindow::RefreshPresetList() {
    ...
    for (int i = 0; i < p->m_nPresets; i++) {
        ...
        if (a && a->flags) {
            ...
            if (a->flags & PFLAG_CANVAS) display += L"\x25A3";  // canvas-sensitive
...
void PresetsWindow::DoBuildControls() {
    ...
    // Nav row: < > clipboard  Up  Into  Subdirs  Filter
    {
        ...
        if (m_hCopyFont) { DeleteObject(m_hCopyFont); m_hCopyFont = NULL; }
        m_hCopyFont = CreateFontW(-(lineH - 4), 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE,
            DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
            CLEARTYPE_QUALITY, DEFAULT_PITCH | FF_DONTCARE, L"Segoe MDL2 Assets");
        m_hBtnCopy = CreateBtn(hw, L"\xE8C8", IDC_MW_PRESET_COPY, x + 2 * (btnW + btnGap), y, btnW, lineH + 4,
            m_hCopyFont ? m_hCopyFont : hFont);
        ...
        if (hTip && m_hBtnCopy) {
            ...
            ti.hwnd = hw;
            ti.uId = (UINT_PTR)m_hBtnCopy;
            ti.lpszText = (LPWSTR)L"Copy current preset name";
    ...
    m_hLblSens = CreateLabel(hw, L"Audio Gain (1=default, >1=boost):", x, y, slw, lineH, hFont);
...
LRESULT PresetsWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
    ...
    if (code == 0 && m_nContextSel >= 0 && m_nContextSel < p->m_nPresets) {
        ...
        if (id == IDC_MW_ANNOT_VIEWERR) {
            ...
            ShowShaderErrorDialog(p, hWnd, full, a ? a->errorText : std::wstring(),
                                  a ? a->errorTime : std::wstring());
            return 0;
        }
...
LRESULT PresetsWindow::DoContextMenu(HWND hWnd, int x, int y) {
    ...
    int curRating = a ? Engine::AverageRating(*a) : 0;
    for (int r = 5; r >= 0; r--) {
        wchar_t label[32];
    ...
    for (int i = 0; i < (int)_countof(kCanvasChoices); i++) {
        ...
        AppendMenuW(hCanvasMenu, MF_STRING | (curCanvas == kCanvasChoices[i] ? MF_CHECKED : 0),
