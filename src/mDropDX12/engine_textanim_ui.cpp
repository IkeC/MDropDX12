...
//----------------------------------------------------------------------
static void PopulateProfileCombo(HWND hProfileCombo, Engine* p, int selectedIdx,
                                 const wchar_t* firstEntry,
                                 bool bIncludeSimple = false)
{
  if (!hProfileCombo) return;
  SendMessage(hProfileCombo, CB_RESETCONTENT, 0, 0);
  SendMessageW(hProfileCombo, CB_ADDSTRING, 0, (LPARAM)firstEntry);       // 0: Disabled/Default
  if (bIncludeSimple)
    SendMessageW(hProfileCombo, CB_ADDSTRING, 0, (LPARAM)L"(Simple)");    // 1: Simple fixed-size
  SendMessageW(hProfileCombo, CB_ADDSTRING, 0, (LPARAM)L"(Random)");      // 1 or 2: Random
  for (int i = 0; i < p->m_nAnimProfileCount; i++)
    SendMessageW(hProfileCombo, CB_ADDSTRING, 0, (LPARAM)p->m_AnimProfiles[i].szName);
  ...
  SendMessage(hProfileCombo, CB_SETCURSEL, sel, 0);
}

// Reverse: combo selection → profile index
static int ComboToProfileIndex(HWND hProfileCombo, bool bHasSimple = false) {
  int sel = (int)SendMessage(hProfileCombo, CB_GETCURSEL, 0, 0);
...
// Agree with the list about which row is selected after a rebuild.
// DoBuildControls selects row 0 and fills the detail panes from it; the real
// selection has already been restored by the time this runs.
void TextAnimWindow::OnRebuilt() {
  if (!m_hList) return;
  const int sel = ListView_GetNextItem(m_hList, -1, LVNI_SELECTED);
  if (sel >= 0 && sel != m_nSelectedRow) UpdateEditControls(sel);
}
...
void TextAnimWindow::DoBuildControls()
{
  ...
  TrackControl(CreateLabelTip(hw, L"Preset Name:",
      L"Text animation profile used when the preset name is shown.",
      x, y, labelW, lineH, hFont));
  HWND hPresetCombo = CreateCombo(hw, IDC_MW_TEXTANIM_PRESET_COMBO, x + labelW, y,
    comboW + pushW + 4, lineH * 10, hFont);
...

  {
    ...
    cx += editW + 8;
    TrackControl(CreateLabelTip(hw, L"Rand:", L"Random offset added to X.",
                                cx, y, medLbl, lineH, hFont));
    cx += medLbl;
  ...
  // Entry position
  {
    ...
    TrackControl(CreateLabelTip(hw, L"Entry:", L"Entry offset, X.",
                                cx, y, smallLbl, lineH, hFont));
    cx += smallLbl;
    ...
    cx += editW + 8;
    TrackControl(CreateLabelTip(hw, L"Move:", L"Move time, in seconds.",
                                cx, y, medLbl, lineH, hFont));
    cx += medLbl;
  ...
  // Ease mode
  {
    ...
    // 46: a radio spends 5 + max(h/2, 11) of its width on the button itself,
    // which left "Out" four pixels short and drawn as "Ou" (issue 159).
    TrackControl(CreateRadio(hw, L"Out", IDC_MW_TEXTANIM_EASE_OUT, cx, y, MulDiv(46, lineH, 26), lineH, hFont, true, false, true, 1));
    cx += MulDiv(46, lineH, 26) + 8;
...

  {
    ...
    // Short words under a bold "Timing" heading, with the full wording on
    // the tooltip: the row carries four label-and-field pairs and there is
    // no width in it for "Duration:" and "Fade Out:" (issue 159).
    TrackControl(CreateLabelTip(hw, L"Dur:", L"Duration, in seconds.",
                                cx, y, medLbl, lineH, hFont));
    cx += medLbl;
    TrackControl(CreateEdit(hw, L"", IDC_MW_TEXTANIM_DURATION, cx, y, editW, lineH, hFont));
    cx += editW + 4;
    TrackControl(CreateLabelTip(hw, L"In:", L"Fade in, in seconds.",
                                cx, y, medLbl, lineH, hFont));
    cx += medLbl;
    TrackControl(CreateEdit(hw, L"", IDC_MW_TEXTANIM_FADEIN, cx, y, editW, lineH, hFont));
    cx += editW + 4;
    TrackControl(CreateLabelTip(hw, L"Out:", L"Fade out, in seconds.",
                                cx, y, medLbl, lineH, hFont));
    cx += medLbl;
  ...
  {
    int cx = x;
    // 61 rather than 55: "Shadow:" did not fit at 55 and the three words in
    // this row are already the shortest that mean anything (issue 159).
    int medLbl = MulDiv(61, lineH, 26);
  ...
  // Randomization flags
  {
    ...
    cx += chkW + 2;
    TrackControl(CreateCheck(hw, L"Growth", IDC_MW_TEXTANIM_RAND_GROWTH, cx, y, chkW + MulDiv(6, lineH, 26), lineH, hFont, false));
    cx += chkW + 2;
...
LRESULT TextAnimWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam)
{
  ...
  case IDC_MW_TEXTANIM_EXPORT:
  {
    ...
    CopyTo(szInitDir, p->m_szBaseDir);
  ...
  case IDC_MW_TEXTANIM_PREVIEW:
    if (m_nSelectedRow >= 0 && m_nSelectedRow < p->m_nAnimProfileCount) {
      ...
      if (!previewText[0]) CopyTo(previewText, L"Preview Animation");

      int slot = p->GetNextFreeSupertextIndex();
      CopyTo(p->m_supertexts[slot].szTextW, previewText);
  ...
  case IDC_MW_TEXTANIM_CHOOSE_FONT:
    if (m_nSelectedRow >= 0 && m_nSelectedRow < p->m_nAnimProfileCount) {
      ...
      LOGFONTW lf = {};
      CopyTo(lf.lfFaceName, prof.szFontFace[0] ? prof.szFontFace : L"Segoe UI");
      ...
      if (ChooseFontW(&cf)) {
        CopyTo(prof.szFontFace, lf.lfFaceName);
