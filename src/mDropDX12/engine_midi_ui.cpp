...
void MidiWindow::SaveEditControls()
{
  ...
  if (m_hLabelEdit) {
    ...
    CopyToA(row.szLabel, utf8.c_str());
  }
  ...
  if (m_hActionCombo) {
    if (row.actionType == MIDI_TYPE_BUTTON) {
      ...
      CopyToA(row.szActionText, utf8.c_str());
    } else if (row.actionType == MIDI_TYPE_KNOB) {
...
void MidiWindow::DoBuildControls()
{
  ...
  m_hDeviceCombo = CreateCombo(hw, IDC_MW_MIDI_DEVICE, x + labelW, y, comboW, lineH * 8,
  ...
  m_hTypeCombo = CreateCombo(hw, IDC_MW_MIDI_TYPE, x + editLabelW + 4, y, typeW,
    lineH * 4, hFont, CBS_DROPDOWNLIST);
  ...
  m_hActionCombo = CreateCombo(hw, IDC_MW_MIDI_ACTION, actionX + actionLabelW + 4, y,
    rw - (actionX - x) - actionLabelW - 4, lineH * 10, hFont, CBS_DROPDOWN | WS_VSCROLL);
  TrackControl(m_hActionCombo);
  y += lineH + gap;

  // Row 3: Increment (for knobs, initially hidden) + buffer
  int incLabelW = MulDiv(78, lineH, 26);   // "Increment:" needs it (issue 159)
