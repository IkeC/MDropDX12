...
// A -child instance pins the shield for its whole life. It is separate from the
// testing-mode shield because the two nest: a TESTING_MODE=0 arriving over IPC
// must not hand a child write access to the settings file its parent owns.
std::atomic<bool> g_shieldPinned{false};

void PinConfigWriteShield() {
  ConfigFlushAll();          // whatever is pending is the user's, not ours
  g_shieldPinned.store(true);
}

bool IsConfigWriteShieldPinned() { return g_shieldPinned.load(); }
...
void SetConfigWriteShield(bool on) {
  ...
  if (!on && g_shieldPinned.load()) {
    // Pinned: the shield never actually comes down, and the overlay has to
    // stay, because it holds everything this child has "written" this session
    // and its own reads are served from it.
    return;
  }
...
bool IsConfigWriteShielded() {
  return g_shield.load() || g_shieldPinned.load();
}
...
std::vector<std::wstring> ConfigStore::SectionNames() {
  if (!m_backend) return {};
  // Flushed first, for the same reason GetSectionNamesTo flushes: the caller is
  // about to read structure that our pending writes change.
  Flush();
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  std::vector<std::wstring> out = m_backend->SectionNames();
  // A section that exists only in the shield's overlay is invisible to the
  // backend. A migration enumerating to REMOVE sections must still see it, or
  // it would leave one behind and report success.
  for (const auto& kv : m_shadow) {
    bool have = false;
    for (const std::wstring& s : out)
      if (_wcsicmp(s.c_str(), kv.first.c_str()) == 0) { have = true; break; }
    if (!have) out.push_back(kv.first);
  }
  return out;
}

std::vector<std::wstring> ConfigStore::KeyNames(const wchar_t* section) {
  if (!m_backend || !section) return {};
  Flush();
  std::lock_guard<std::recursive_mutex> lock(m_mutex);
  std::vector<std::wstring> out = m_backend->KeyNames(section);
  // Keys living only in the shield's overlay, for the same reason
  // SectionNames merges them: a migration must not leave one behind.
  auto it = m_shadow.find(section);
  if (it != m_shadow.end()) {
    for (const auto& kv : it->second) {
      bool have = false;
      for (const std::wstring& k : out)
        if (_wcsicmp(k.c_str(), kv.first.c_str()) == 0) { have = true; break; }
      if (!have) out.push_back(kv.first);
    }
  }
  return out;
...
bool ConfigStore::SetFloat(const wchar_t* section, const wchar_t* key, float value) {
  ...
  FormatToLocale(text, L"%f", g_use_C_locale, value);
  return Store(section, key, text);
}
