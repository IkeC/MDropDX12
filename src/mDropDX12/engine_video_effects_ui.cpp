...
void VideoEffectsWindow::BuildTransformPage(int x, int y, int rw, int lineH, int gap)
{
    ...
    HWND hBlend = CreateCombo(hw, IDC_MW_VFX_BLENDMODE, x + slLbl + 4, y, rw - slLbl - 4,
      lineH * 8, hFont);
...
void VideoEffectsWindow::BuildAudioPage(int x, int y, int rw, int lineH, int gap)
{
    ...
    for (auto& r : rows) {
        ...
        HWND hSrcCombo = CreateCombo(hw, r.comboID, x + lblW + 4, y, comboW, lineH * 6, hFont);
        SendMessageW(hSrcCombo, CB_ADDSTRING, 0, (LPARAM)L"None");
        SendMessageW(hSrcCombo, CB_ADDSTRING, 0, (LPARAM)L"Bass");
        SendMessageW(hSrcCombo, CB_ADDSTRING, 0, (LPARAM)L"Mid");
        SendMessageW(hSrcCombo, CB_ADDSTRING, 0, (LPARAM)L"Treb");
        SendMessageW(hSrcCombo, CB_ADDSTRING, 0, (LPARAM)L"Vol");
        SendMessage(hSrcCombo, CB_SETCURSEL, r.link->source, 0);
        P_TC(2, hSrcCombo);
...
void Engine::InitVFXProfileStore()
{
    ...
    // Onto profiles/vfx/ before anything reads a profile out of the store.
    m_vfxProfiles.MigrateSingleFile();
    ...
    // Onto profiles/audio/ BEFORE EnsureBuiltIns, or the built-ins would be
    // written into the new directory and then the migration would find the old
    // file still holding the user's edited copies of the same names.
    AudioProfiles().MigrateSingleFile();
...
void Engine::PushScopedVFXProfile(const std::wstring& name)
{
    ...
    if (!m_bVFXScopeActive) {
        ...
        CopyTo(m_szVFXScopeProfile, name.c_str());
    }
...
void Engine::PopScopedVFXProfile()
{
    ...
    if (m_bVFXScopeDirty) {
        ...
        //
        // kPrompts suppresses the WINDOW, and only the window. The scope stays
        // active and still waits for an answer, because AnswerScopedVFX is also
        // reachable over IPC (VFX_SCOPED_KEEP / _DISCARD) and a harness driving
        // it deliberately is the one caller that SHOULD still work while
        // testing. Clearing the pending edit here instead was tried and is
        // wrong -- it answered Discard on the harness's behalf, and
        // test_preset_overrides' Keep cases failed exactly as they should have.
        if (!Restrained(restraint::kPrompts) &&
            (!m_pVFXKeepPromptWindow || !m_pVFXKeepPromptWindow->IsOpen()))
...
bool Engine::LoadVideoFXProfile(const wchar_t* name)
{
    ...
    ApplyVFXProfile(d);
    CopyTo(m_szCurrentVFXProfile, name);
    return true;
}
