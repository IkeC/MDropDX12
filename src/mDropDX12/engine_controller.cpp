...
std::string Engine::GetDefaultControllerJSON()
{
    return
        ...
        "//           BLACKOUT, SEND=0x<vk>   (hex; a plain number is typed)\r\n"
...
{
    m_bControllerEnabled = Config().GetInt(L"Controller", L"Enabled", 0) != 0;
    m_nControllerDeviceID = Config().GetInt(L"Controller", L"DeviceID", -1);
    Config().GetStringTo(L"Controller", L"DeviceName", L"", m_szControllerName, 256);
}
...
void Engine::SaveControllerSettings()
{
    ...
    Config().SetString(L"Controller", L"Enabled", m_bControllerEnabled ? L"1" : L"0");
    swprintf(buf, 32, L"%d", m_nControllerDeviceID);
    Config().SetString(L"Controller", L"DeviceID", buf);
    Config().SetString(L"Controller", L"DeviceName", m_szControllerName);
}

// ---------------------------------------------------------------------------
// EnumerateControllers — populate a combo box with available controllers
// ---------------------------------------------------------------------------
void Engine::EnumerateControllers(HWND hDeviceCombo)
{
    ...
    for (UINT id = 0; id < numDevs && id < 16; id++) {
        ...
        if (joyGetDevCapsW(id, &caps, sizeof(caps)) == JOYERR_NOERROR) {
            ...
            if (joyGetPosEx(id, &info) == JOYERR_NOERROR) {
                int idx = (int)SendMessageW(hDeviceCombo, CB_ADDSTRING, 0, (LPARAM)caps.szPname);
                SendMessage(hDeviceCombo, CB_SETITEMDATA, idx, (LPARAM)id);
                found++;
            }
        }
    }
    ...
    if (found == 0) {
        int idx = (int)SendMessageW(hDeviceCombo, CB_ADDSTRING, 0, (LPARAM)L"(No controllers found)");
        SendMessage(hDeviceCombo, CB_SETITEMDATA, idx, (LPARAM)-1);
    }

    // Try to select the saved controller by name
    if (m_szControllerName[0]) {
        int count = (int)SendMessage(hDeviceCombo, CB_GETCOUNT, 0, 0);
        for (int i = 0; i < count; i++) {
            wchar_t name[256];
            SendMessageW(hDeviceCombo, CB_GETLBTEXT, i, (LPARAM)name);
            if (_wcsicmp(name, m_szControllerName) == 0) {
                SendMessage(hDeviceCombo, CB_SETCURSEL, i, 0);
                m_nControllerDeviceID = (int)SendMessage(hDeviceCombo, CB_GETITEMDATA, i, 0);
                return;
            }
        }
    }
    ...
    if (m_nControllerDeviceID >= 0) {
        int count = (int)SendMessage(hDeviceCombo, CB_GETCOUNT, 0, 0);
        for (int i = 0; i < count; i++) {
            if ((int)SendMessage(hDeviceCombo, CB_GETITEMDATA, i, 0) == m_nControllerDeviceID) {
                SendMessage(hDeviceCombo, CB_SETCURSEL, i, 0);
                return;
            }
        }
    ...
    if (found > 0)
        m_nControllerDeviceID = (int)SendMessage(hDeviceCombo, CB_GETITEMDATA, 0, 0);
}
...
{
    // A -child instance polls no controller. joyGetPosEx reads a shared system
    // device rather than a per-window input queue, so parent and child would
    // both observe the same button press and both act on it -- one press, two
    // preset changes. The parent owns physical input and relays whatever it
    // wants a child to do over the named pipe.
    if (m_bChildMode)
        return;
...
void Engine::ExecuteControllerCommand(const std::string& cmdRaw)
{
    ...
    else if (cmd == "MIRROR_WM" || cmd == "MIRROR_WATERMARK") {
        if (hwnd) PostMessage(hwnd, WM_MW_MIRROR_WM, 0, 0);
    }
    else if (cmd == "WATERMARK") {
        if (hwnd) PostMessage(hwnd, WM_MW_WATERMARK, 0, 0);
    }
    else if (cmd == "MIRROR_INDEPENDENT" || cmd == "INDEPENDENT") {
        ToggleMirrorIndependentRender();
    }
    else if (cmd == "ALWAYS_ON_TOP" || cmd == "AOT") {
        if (hwnd) PostMessage(hwnd, WM_MW_ALWAYS_ON_TOP_TOGGLE, 0, 0);
    }
...
static LRESULT CALLBACK ControllerHelpWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT: {
        ...
        const wchar_t* cmdLines[] = {
            ...
            L"SEND=0x1B \u2014 Virtual key, HEX ONLY. A plain number is typed as",
            L"          text instead: SEND=27 types \"27\", it is not ESC.",
        };
