...
void ControllerWindow::DoBuildControls() {
...

  {
    ...
    HWND hCtrlCombo = CreateCombo(hw, IDC_MW_CTRL_DEVICE, x + lblW, y, comboW, lineH * 8,
      hFont, CBS_DROPDOWNLIST | WS_VSCROLL, true, WS_EX_CLIENTEDGE);
    ...
    // Multiline JSON edit control -- eight lines, or the rest of the window if
    // there is more of it.
    //
    // A fixed height leaves whatever the window has spare as a blank strip
    // under the buttons, and issue 157 made that visible: this window's default
    // is now scaled to the font, so at a large font it opens taller and the
    // strip was about a fifth of it. The button row keeps its own space and the
    // edit takes the difference, which is the same shape the Remote window's
    // IPC list uses.
    //
    // Only ever GROWS from the eight-line design height. A window too small for
    // that leaves the buttons hanging over the edge, which is exactly what
    // FitClippedContentsOnOpen is for -- and letting the edit shrink instead
    // would hide the overflow from it.
    int editH = lineH * 8;
    {
      RECT rcClient = {};
      GetClientRect(hw, &rcClient);
      const int reserve = lineH + gap + 8;   // the button row, and a foot
      const int avail = rcClient.bottom - y - reserve;
...
LRESULT ControllerWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
  ...
  if (id == IDC_MW_CTRL_ENABLE && code == BN_CLICKED) {
    ...
    if (bChecked) {
      HWND hJsonEdit = GetDlgItem(hWnd, IDC_MW_CTRL_JSON_EDIT);
      if (hJsonEdit) {
        int len = GetWindowTextLengthW(hJsonEdit);
        std::wstring wText(len + 1, L'\0');
        GetWindowTextW(hJsonEdit, &wText[0], len + 1);
  ...
  if (id == IDC_MW_CTRL_SCAN && code == BN_CLICKED) {
    HWND hDeviceCombo = GetDlgItem(hWnd, IDC_MW_CTRL_DEVICE);
    if (hDeviceCombo) p->EnumerateControllers(hDeviceCombo);
    return 0;
  }
  ...
  if (id == IDC_MW_CTRL_DEFAULTS && code == BN_CLICKED) {
    ...
    if (hJsonEdit) {
      ...
      SetWindowTextW(hJsonEdit, wDef.c_str());
    }
    return 0;
  }
  ...
  if (id == IDC_MW_CTRL_DEVICE && code == CBN_SELCHANGE) {
    ...
    if (sel >= 0) {
      int joyID = (int)SendMessage(hDeviceCombo, CB_GETITEMDATA, sel, 0);
      p->m_nControllerDeviceID = joyID;
      p->m_dwLastControllerButtons = 0;
      SendMessageW(hDeviceCombo, CB_GETLBTEXT, sel, (LPARAM)p->m_szControllerName);
      p->SaveControllerSettings();
    }
    return 0;
  }
