...
enum class RenderCmd : uint8_t {
    SetMeshSize,         // Adopt Engine::m_reqGridX/m_reqGridY, then ResetBufferAndFonts().
                         // Carries no payload ON PURPOSE: that makes it idempotent,
                         // so the ~23 of these a slider drag queues collapse to one
                         // real resize instead of replaying every intermediate size.
                         // m_nGridX/m_nGridY size the m_verts allocation AND index
                         // it, so they may only be written by the thread that
                         // reads them. Assigning from a ToolWindow and posting the
                         // realloc afterwards left the render thread indexing the
                         // OLD allocation with the NEW dimensions -- at 64 -> 192
                         // about 24,800 MYVERTEX past the end, across up to nine
                         // threads. See forgejo#8.
    ...
    NextPresetLocal,     // LoadRandomPreset(fParam) -- no child broadcast
    PrevPresetLocal,     // PrevPresetLocal(fParam)  -- no child broadcast
    // Undo what a testing run left in memory, then lower the config write
    // shield. Render-thread only: it rewrites m_displayOutputs and retires
    // child processes, the same reasons LoadDisplayProfile is queued.
    RestoreAfterTesting,
    ...
    LoadDisplayProfile,  // LoadDisplayProfile(sParam). Rewrites m_displayOutputs,
                         // spawns and retires children and saves settings, none
                         // of which may run on the IPC or a ToolWindow thread
                         // while the render thread walks the same vector.
    AddSpoutOutput,      // Insert a new, disabled Spout output at the front of
                         // m_displayOutputs, then refresh the Displays tab.
                         // Same reason as LoadDisplayProfile: #27.
    RemoveDisplayOutput, // iParam1 = index into m_displayOutputs. Destroy and
                         // erase that Spout output, re-validated on this
                         // thread since the selection may be stale by the
                         // time the command is drained. #27.
    ResetDisplayOutputs, // ResetDisplayOutputsToDefaults(). Rewrites
                         // m_displayOutputs and destroys every monitor
                         // mirror -- render-thread only, #27.
    Quit,                // Clean shutdown
};
