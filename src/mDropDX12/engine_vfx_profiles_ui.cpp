...
void VFXProfileWindow::DoBuildControls()
{
    ...
    HWND hList = CreateListBox(m_hWnd, IDC_MW_VFXP_LIST, x, y, rw, listH, m_hFont,
...
void VFXProfileWindow::ApplySelectedProfile()
{
    ...
    if (m_pEngine->m_bEnableVFXStartup)
        CopyTo(m_pEngine->m_szVFXStartup, m_pEngine->m_szCurrentVFXProfile);
...
LRESULT VFXProfileWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam)
{
    ...
    case IDC_MW_VFXP_SAVE: {
        ...
        if (m_pEngine->SaveVideoFXProfile(name.c_str())) {
            ...
            if (m_pEngine->m_bEnableVFXStartup)
                CopyTo(m_pEngine->m_szVFXStartup, name.c_str());
            m_pEngine->SaveSpoutInputSettings();
            m_pEngine->OnVideoFXChanged();
            RefreshProfileList();
        }
    ...
    case IDC_MW_VFXP_IMPORT: {
        ...
        if (n == 1 && !suggested.empty()) {
            ...
            wchar_t msg[512];
            FormatTo(msg, L"Import %d profile(s) from this file?\n\n"
        ...
        if (done > 0) {
            RefreshProfileList();
            wchar_t msg[128];
            FormatTo(msg, L"Imported %d profile(s).", done);
    ...
    case IDC_MW_VFXP_DELETE: {
        ...
        wchar_t msg[512];
        FormatTo(msg, L"Delete profile \"%s\"?", name.c_str());
