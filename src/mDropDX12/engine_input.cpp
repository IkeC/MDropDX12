...
void Engine::SetFPSCap(int fps) {
  ...
  m_max_fps_dm = fps;
  m_max_fps_w = fps;
  Config().SetInt(L"Settings", L"max_fps_fs", fps);
  Config().SetInt(L"Settings", L"max_fps_dm", fps);
  Config().SetInt(L"Settings", L"max_fps_w", fps);
  ...
  if (hSettingsWnd && IsWindow(hSettingsWnd)) {
    HWND hFpsCombo = GetDlgItem(hSettingsWnd, IDC_MW_FPS_CAP);
    if (hFpsCombo && idx >= 0)
      SendMessage(hFpsCombo, CB_SETCURSEL, idx, 0);
  }
  // Drop poisoned FPS history when switching cap ↔ unlimited (avoids HUD stuck
  // at ~60 while animation races from 1/m_fps * real_frame_rate).
...
void copyStringToClipboardA(const char* source) {
  ...
  lstrcpynA(buffer, source, lstrlenA(source) + 1);
...
void copyStringToClipboardW(const wchar_t* source) {
  ...
  lstrcpynW(buffer, source, lstrlenW(source) + 1);
  GlobalUnlock(clipbuffer);
...
void Engine::ToggleAlwaysOnTop(HWND hwnd) {
  // Applies m_bAlwaysOnTop to the render window and any live mirror windows.
  // (Does not flip the flag — callers set m_bAlwaysOnTop first.)
  HWND z = m_bAlwaysOnTop ? HWND_TOPMOST : HWND_NOTOPMOST;

  if (hwnd && IsWindow(hwnd)) {
    RECT rect;
    GetWindowRect(hwnd, &rect);
    SetWindowPos(hwnd, z, rect.left, rect.top,
                 rect.right - rect.left, rect.bottom - rect.top,
                 SWP_DRAWFRAME | SWP_FRAMECHANGED);
  }

  // Mirror mode used to force TOPMOST on every mirror regardless of this setting,
  // so "Always On Top off" appeared broken while secondary screens stayed topmost.
  for (auto& out : m_displayOutputs) {
    if (out.config.type != DisplayOutputType::Monitor || !out.monitorState)
      continue;
    HWND hMirror = out.monitorState->hWnd;
    if (hMirror && IsWindow(hMirror))
      SetWindowPos(hMirror, z, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOACTIVATE);
  }
}

void Engine::RaiseMirrorSurfaces(HWND hPrimary)
{
  // CRITICAL: Mirror HWNDs are owned by the *render* thread; the primary is owned by
  // the *UI/message* thread. SetWindowPos/SetForegroundWindow across threads can
  // SendMessage to the owner and deadlock (looks like a freeze). Only touch HWNDs
  // owned by the calling thread; set flags so the other side raises later.
  const DWORD thisTid = GetCurrentThreadId();

  auto raiseIfOwned = [this, thisTid](HWND h, bool activate) {
    if (!h || !IsWindow(h)) return;
    DWORD pid = 0;
    DWORD tid = GetWindowThreadProcessId(h, &pid);
    if (tid != thisTid)
      return; // wrong thread — would deadlock
    // No SWP_SHOWWINDOW (can re-enter); no activate on foreign focus races
  ...
  // Raise mirrors only when this is the render thread (window creator)
  for (auto& out : m_displayOutputs) {
    if (out.config.type != DisplayOutputType::Monitor || !out.monitorState)
      continue;
    HWND hMirror = out.monitorState->hWnd;
    if (!hMirror || !IsWindow(hMirror))
      continue;
    if (m_bMirrorsActive && out.config.bEnabled && !out.bSkippedSameMonitor)
      raiseIfOwned(hMirror, false);
  }

  // Raise primary only when this is the UI thread
  if (hPrimary && IsWindow(hPrimary)) {
    raiseIfOwned(hPrimary, true);
    DWORD pid = 0;
    if (GetWindowThreadProcessId(hPrimary, &pid) == thisTid) {
      // Async foreground — never block the render thread.
      //
      // Not for a quiet launch. Startup raises mirrors, and this is one of the
      // activations that is NOT in App.cpp -- which is why suppressing every
      // SetForegroundWindow there left the foreground still being taken.
      if (!m_bQuietLaunch)
        SetForegroundWindow(hPrimary);
    }
  }
}
...
static void ScanFolderForPresets(const wchar_t* folderPath, std::vector<std::wstring>& out) {
  ...
  for (const wchar_t* pat : patterns) {
    ...
    CopyTo(searchPath, folderPath);
    AppendTo(searchPath, pat);
    WIN32_FIND_DATAW fd;
...
void LoadPresetFilesViaDragAndDrop(WPARAM wParam) {
  ...
  if (totalItems == 1 && folders.size() == 1) {
    ...
    if (dir.back() != L'\\') dir += L'\\';
    CopyTo(g_engine.m_szPresetDir, dir.c_str());
  ...
  CopyTo(g_engine.m_szPresetDir, dndDir);
  Config().SetString(L"Settings", L"szPresetDir", g_engine.m_szPresetDir);
  g_engine.UpdatePresetList(true, true);
...
LRESULT Engine::MyWindowProc(HWND hWnd, unsigned uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_MW_SETSPOUTSENDER:
    // Still not implemented — absorb to prevent DefWindowProc handling.
    // COVER_CHANGED: the engine already self-detects artwork changes via
    // polling and pushes SIGNAL|COVER_CHANGED OUTWARD (App.cpp) rather than
    // consuming an inbound one; #29's own thread left this direction
    // deliberately ambiguous rather than invent behavior for it.
    // SETSPOUTSENDER: not reachable from the IPC signal table at all (no
    // "SETSPOUTSENDER" entry in pipe_server.cpp's s_signalTable) and out of
    // #29's scope, which covers ENABLESPOUTMIX/SET_INPUTMIX_* instead
    // (already answered by GET_INPUT_MIX).
    return 0;
  // Absolute setters, not the 'K' hotkey's toggle -- SPRITE_MODE and
  // MESSAGE_MODE are two distinct signals precisely so a remote can assert
  // the state it wants instead of guessing which way a toggle will land.
  // Each mirrors HK_SPRITE_MODE's own branch (engine_hotkeys.cpp) exactly,
  // including the classic-Remote notification, so a hotkey press and an IPC
  // signal are indistinguishable in their effect. #29.
  ...
  case WM_MW_SETVIDEODEVICE: {
    // wParam = 1-based index into EnumerateWebcams(), matching
    // IDC_MW_VIDINPUT_WEBCAM's combo (its own index 0 is "(Default)", so a
    // signal's wParam==0 means the same thing: use the system default).
    // Sets the device only -- does not switch the active source, matching
    // that combo's own handler, which only re-inits capture when webcam is
    // ALREADY the active source. #29.
    auto webcams = VideoCaptureSource::EnumerateWebcams();
    const int idx = (int)wParam;
    if (idx <= 0 || idx > (int)webcams.size())
      m_szWebcamDevice[0] = L'\0';
    else
      wcsncpy_s(m_szWebcamDevice, webcams[idx - 1].name.c_str(), _TRUNCATE);
  ...
  case WM_MW_ENABLEVIDEOMIX: {
    // wParam = VID_SOURCE_* (0 none, 1 Spout, 2 webcam, 3 file) -- the same
    // numbering IDC_MW_VIDINPUT_SOURCE's combo already uses and GET_INPUT_MIX
    // already reports as source=. Mirrors that combo's CBN_SELCHANGE handler
    // (engine_displays_ui.cpp) exactly, minus the UI EnableWindow calls,
    // which have no meaning for a headless signal. #29.
    const int newSrc = (int)wParam;
    if (newSrc < VID_SOURCE_NONE || newSrc > VID_SOURCE_FILE) return 0;
    const int oldSrc = m_nVideoInputSource;
    if (newSrc != oldSrc) {
      if (oldSrc == VID_SOURCE_SPOUT) DestroySpoutInput();
      else if (oldSrc == VID_SOURCE_WEBCAM || oldSrc == VID_SOURCE_FILE) DestroyVideoCapture();

      m_nVideoInputSource = newSrc;
      m_bSpoutInputEnabled = (newSrc != VID_SOURCE_NONE);

      if (newSrc == VID_SOURCE_SPOUT) InitSpoutInput();
      else if (newSrc == VID_SOURCE_WEBCAM || newSrc == VID_SOURCE_FILE) InitVideoCapture();
  ...
  case WM_MW_IPC_MESSAGE:
  {
    ...
    if (message) {
      ...
      FormatTo(g_szLastIPCTime, L"%02d:%02d:%02d", st.wHour, st.wMinute, st.wSecond);
      wcsncpy_s(g_szLastIPCMessage, message, _TRUNCATE);
      g_lastIPCMessageSeq.fetch_add(1);

      // 1 = ordinary, 3 = the connection asked for STRICT dispatch. The pipe
      // server chose it per CLIENT and put it in wParam because a post is
      // asynchronous -- see PipeServer::DispatchMessage (issue 102).
      if (dwData == 1 || dwData == 3) {
        RenderCommand cmd;
        cmd.cmd = RenderCmd::IPCMessage;
        cmd.sParam = message;
        cmd.iParam1 = (dwData == 3) ? 1 : 0;
        EnqueueRenderCmd(std::move(cmd));
      }
  ...
  case WM_CHAR:   // plain & simple alphanumeric keys
    ...
    if (m_waitstring.bActive)	// if user is in the middle of editing a string
    {
      if ((wParam >= ' ' && wParam <= 'z') || wParam == '{' || wParam == '}') {
            ...
            wParam == '&'))	// NOTE: '&' is legal in filenames, but we try to avoid it since during GDI display it acts as a control code (it will not show up, but instead, underline the character following it).
        {
          // illegal char
          wchar_t msg[64];
          AddError(wasabiApiLangString(IDS_ILLEGAL_CHARACTER, msg), m_ErrorDuration, ERR_MISC, true);
        }
        else if (len + nRepeat >= m_waitstring.nMaxLen) {
          // m_waitstring.szText has reached its limit
          wchar_t msg[64];
          AddError(wasabiApiLangString(IDS_STRING_TOO_LONG, msg), m_ErrorDuration, ERR_MISC, true);
        }
        else {
          ...
          if (m_waitstring.bDisplayAsCode) {
            ...
            if (m_waitstring.bOvertypeMode) {
              ...
              for (int rep = 0; rep < nRepeat; rep++) {
                if (m_waitstring.nCursorPos == len) {
                  AppendToA(m_waitstring.szCode, buf);
                  len++;
                }
                else {
            ...
            if (m_waitstring.bOvertypeMode) {
              ...
              for (int rep = 0; rep < nRepeat; rep++) {
                if (m_waitstring.nCursorPos == len) {
                  AppendTo(m_waitstring.szText, buf);
                  len++;
                }
                else
    ...
    else if (m_UI_mode == UI_LOAD_DEL)	// waiting to confirm file delete
    {
      if (wParam == 'y' || wParam == 'Y')	// 'y' or 'Y'
      {
        ...
        FormatTo(szDelFile, L"%s%s", GetPresetDir(), PresetNameAt(m_nPresetListCurPos).c_str());
    ...
    else if (m_UI_mode == UI_SAVE_OVERWRITE)	// waiting to confirm overwrite file on save
    {
      if (wParam == 'y' || wParam == 'Y')	// 'y' or 'Y'
      {
        ...
        FormatTo(szNewFile, L"%s%s.milk", GetPresetDir(), m_waitstring.szText);
  ...
  case WM_MOUSEWHEEL:
  {
    // Shift+wheel → opacity (UI-thread safe: layered-window attributes only).
    // Plain wheel → next/prev preset (must run on the render thread — LoadPreset
    // mutates GPU/shader state and was a crash when called from WndProc).
    const short zDelta = GET_WHEEL_DELTA_WPARAM(wParam);
    if (zDelta == 0)
      return 0;

    if (bShiftHeldDown) {
      ToggleWindowOpacity(hWnd, zDelta < 0); // down = more transparent
      return 0;
    }
    ...
    if (zDelta < 0) {
      cmd.cmd = RenderCmd::NextPreset;
      cmd.fParam = 0.0f; // hard cut (same as previous direct NextPreset(0))
    } else {
      cmd.cmd = RenderCmd::PrevPreset;
      cmd.fParam = 0.0f;
    }
      ...
      if (hSW) {
        ...
        if (hList) {
          ...
          for (int i = 0; i < m_nPresets; i++) {
            if (PresetNameAt(i).empty()) continue;
            SendMessageW(hList, LB_ADDSTRING, 0, (LPARAM)PresetNameAt(i).c_str());
          }
  ...
  case WM_SYSKEYDOWN:
    // A key pressed while ALT is held arrives HERE and never as WM_KEYDOWN,
    // so every local binding carrying the Alt modifier reached no lookup at
    // all and simply did nothing. F10 arrives here too, unmodified, and was
    // equally unreachable. Found by binding Alt+B and pressing it.
    //
    // Only the configurable table is consulted, and an unmatched key returns
    // 1 so the rest of Alt still works: Alt+Enter, Alt+S and Alt+F4 are
    // handled by the window proc that forwarded this message, and the system
    // menu needs DefWindowProc to see what is left.
    //
    // The gates are the same three, in the same order, as WM_KEYDOWN below:
    // a child forwards before anything else because it pins testing mode on
    // for its whole life, and testing mode swallows the keyboard so a stray
    // keystroke cannot alter a measurement.
    if (m_UI_mode == UI_REGULAR) {
      UINT mods = 0;
      if (GetKeyState(VK_CONTROL) & 0x8000) mods |= MOD_CONTROL;
      if (GetKeyState(VK_SHIFT) & 0x8000)   mods |= MOD_SHIFT;
      // Bit 29 is the message's own context code, which Windows sets when
      // ALT was down. Taken as well as GetKeyState, not instead: the key
      // state is the truth for a real press, and the context code is what
      // makes this path drivable by a posted message -- so the Alt
      // bindings can be tested without a window having to take the focus.
      if ((GetKeyState(VK_MENU) & 0x8000) || (lParam & (1 << 29)))
        mods |= MOD_ALT;
      if (TestingModeHandleKey((UINT)wParam)) return 0;
  ...
  case WM_KEYDOWN:    // virtual-key codes
    ...
    if (m_UI_mode == UI_REGULAR) {
      ...
      // Testing mode swallows the keyboard so a stray keystroke cannot alter a
      // measurement. It is gated HERE, at the keyboard entry, and not inside
      // DispatchHotkeyAction -- IPC's ACTION= goes through that same function
      // and must keep working, since it is how the harness drives the app.
      if (TestingModeHandleKey((UINT)wParam)) return 0;
      if (LookupLocalHotkey((UINT)wParam, mods)) return 0;
    }
...
// case 1: waitstring mode
    if (m_waitstring.bActive) {
      ...
      // bShiftHeldDown/bCtrlHeldDown come from the top of MyWindowProc; they were
      // re-read here from the same GetKeyState calls, which yields the same answer
      ...
      else	// waitstring mode key pressed, and ctrl NOT held down
      {
        ...
        case VK_RETURN:
          if (m_UI_mode == UI_LOAD_RENAME)	// rename (move) the file
          {
            ...
            wchar_t szNewFile[512];
            CopyTo(szOldFile, GetPresetDir());
            CopyTo(szNewFile, GetPresetDir());
            AppendTo(szOldFile, PresetNameAt(m_nPresetListCurPos).c_str());
            AppendTo(szNewFile, m_waitstring.szText);
            AppendTo(szNewFile, L".milk");
            ...
            if (!ret) {
              ...
              if (m_UI_mode == UI_IMPORT_WAVE || m_UI_mode == UI_IMPORT_SHAPE)
                wasabiApiLangString(IDS_ERROR_IMPORTING_BAD_FILENAME, buf);
              else
                wasabiApiLangString(IDS_ERROR_IMPORTING_BAD_FILENAME_OR_NOT_OVERWRITEABLE, buf);
              AddError(buf, m_ErrorDuration, ERR_MISC, true);
            }
    ...
    case VK_DELETE:
      if (m_UI_mode == UI_LOAD) {
        if (PresetNameAt(m_nPresetListCurPos).c_str()[0] != '*')	// can't delete directories
      ...
      else //if (m_nNumericInputDigits == 0)
      {
        if (m_nNumericInputMode == NUMERIC_INPUT_MODE_CUST_MSG) {
          ...
          // shift/ctrl come from the top of MyWindowProc — same GetKeyState reads,
          // same message, so re-reading them here made no difference
    ...
    case VK_INSERT:		// RENAME
      if (m_UI_mode == UI_LOAD) {
        if (PresetNameAt(m_nPresetListCurPos).c_str()[0] != '*')	// can't rename directories
        {
          // go into RENAME mode
          ...
          CopyTo(m_waitstring.szText, PresetNameAt(m_nPresetListCurPos).c_str());
          RemoveExtension(m_waitstring.szText);

          // set the prompt & 'tooltip'
          FormatResTo(m_waitstring.szPrompt, IDS_ENTER_THE_NEW_NAME_FOR_X, m_waitstring.szText);
    ...
    case VK_RETURN:
      ...
      HitEnterFromLoadMenu:
        ...
        if (PresetNameAt(m_nPresetListCurPos).c_str()[0] == '*') {
          // CHANGE DIRECTORY
          wchar_t* p = GetPresetDir();

          if (wcscmp(PresetNameAt(m_nPresetListCurPos).c_str(), L"*..") == 0) {
            // back up one dir
            ...
            if (p2 && p2 > p) {
              *p2 = 0;
              p2 = wcsrchr(p, L'\\');
              if (p2) *(p2 + 1) = 0;
              else AppendTo(m_szPresetDir, L"\\");  // keep drive root as "X:\"
            }
          }
          else {
            // open subdir
            AppendTo(m_szPresetDir, &PresetNameAt(m_nPresetListCurPos).c_str()[1]);
            AppendTo(m_szPresetDir, L"\\");
          }

          Config().SetString(L"Settings", L"szPresetDir", GetPresetDir());
          ...
          // Distinguished from the other keyboard paths because this one is
          // the browser, and the browser being open at all is information: a
          // report of "it changed while locked" turned out to be loads from
          // here, which the lock does not gate because they are asked for.
          m_pszNextLoadReason = (wParam == VK_SPACE) ? "browser-space"
                                                     : "browser-enter";
...
void Engine::WaitString_Paste() {
  ...
  if (m_waitstring.bActive) {
    ...
    if (m_waitstring.bDisplayAsCode) {
      char tmp[64000];
      CopyToA(tmp, getStringFromClipboardA());
      ConvertCRsToLFCA(tmp, m_waitstring.szClipboard);
    }
    else {
      wchar_t tmp[64000];
      CopyTo(tmp, getStringFromClipboardW());
...
void Engine::ExecuteRenderCommand(const RenderCommand& cmd) {
  switch (cmd.cmd) {
    case RenderCmd::ResetBuffers:
      ResetBufferAndFonts();
      break;
    case RenderCmd::SetMeshSize: {
      // Clamped here rather than trusted from the sender, because this is the
      // only place the value becomes visible to the code that indexes m_verts.
      const int gx = max(8, min(MAX_GRID_X, m_reqGridX.load()));
      const int gy = max(6, min(MAX_GRID_Y, m_reqGridY.load()));
      // Already there: a later request in the same drag was applied by an
      // earlier drain of this queue. Skipping is what keeps a drag from
      // rebuilding every buffer in the engine once per slider tick.
      if (gx == m_nGridX && gy == m_nGridY)
        break;
      DLOG_INFO("Mesh resize: %dx%d -> %dx%d", m_nGridX, m_nGridY, gx, gy);
      m_nGridX = gx;
      m_nGridY = gy;
      ResetBufferAndFonts();
      break;
    ...
    case RenderCmd::RefreshDisplays:
      // Destroy only after GPU idle (DestroyDisplayOutput WaitForGpu) — rare path
      // (Windows "make this my main display" / hotkey primary switch fires WM_DISPLAYCHANGE)
      for (auto& out : m_displayOutputs) {
        if (out.config.type == DisplayOutputType::Monitor && out.monitorState)
    ...
    case RenderCmd::NextPreset:
      // fParam is blend time; callers set m_fBlendTimeUser or 0 for hard cut.
      //
      // Tagged "command" rather than by sender: RenderCmd carries no origin,
      // so IPC, a hotkey, the button board and the remote are indistinguishable
      // by the time they arrive here. That still separates "something asked for
      // this" from the self-acting paths (timer, hardcut-*, song-change), which
      // is the distinction a log reader needs first.
      //
      // NextPreset(), NOT LoadRandomPreset() -- they were interchangeable until
      // NextPreset also began nudging the per-display children (forgejo#22),
      // and this is the path SIGNAL|NEXT_PRESET takes. Calling the inner one
      // meant a remote pressing Next moved the primary and left every
      // independent display where it was; the children obeyed the signal
      // perfectly when it was sent to them directly, which is what made this
      // look like a child-side problem.
      if (!m_bPresetLockedByCode) {
        m_pszNextLoadReason = "command";
        NextPreset(cmd.fParam);
      }
      break;
    case RenderCmd::PrevPreset:
      ...
      break;
    // Addressed at ONE display. The broadcasting forms above would step every
    // independent display too, which is the opposite of what an addressed
    // command means.
    case RenderCmd::NextPresetLocal:
      if (!m_bPresetLockedByCode) {
        m_pszNextLoadReason = "command-local";
        LoadRandomPreset(cmd.fParam);
      }
      break;
    case RenderCmd::PrevPresetLocal:
      if (!m_bPresetLockedByCode) {
        m_pszNextLoadReason = "command-local";
        PrevPresetLocal(cmd.fParam);
      }
      m_fHardCutThresh *= 2.0f;
      break;
    case RenderCmd::LoadPreset:
      ...
      break;
    case RenderCmd::LoadPresetPath:
      // ToolWindows run their own message pumps on their own threads, so they
      // must never call LoadPreset directly: it drives m_presetLoadThread and
      // m_pNewState/m_NewShaders, which the render thread owns. Two threads
      // joining/assigning one std::thread is what crashed the app (abort() via
      // std::terminate). Route the request here instead.
      if (!cmd.sParam.empty()) {
        if (cmd.iParam1 >= 0 && cmd.iParam1 < m_nPresets) {
          m_nCurrentPreset = cmd.iParam1;
        } else {
          // Every current caller of the path form (ApplyPrimaryProfilePreset,
          // SET_DISPLAY_PRESET=<primary>,<path>) sends iParam1=-1 -- they have
          // a path, not an index, because they run on threads that must not
          // touch m_presets[] (see above). Left unresolved, m_presets[] and
          // m_nCurrentPreset stayed wherever the LAST rescan/step left them:
          // LoadPreset below still plays the right file (it takes the raw
          // path), but the NEXT NextPreset()/PrevPreset() steps from that
          // stale index into a stale, possibly wrong-directory list --
          // usually landing on a filename that doesn't exist at the new
          // directory, which LoadPreset silently no-ops on (forgejo#93).
          //
          // Rescans by calling UpdatePresetList directly rather than going
          // through ChangePresetDir (what the PRESET= IPC handler uses,
          // engine_messages.cpp): ChangePresetDir also writes szPresetDir to
          // settings.ini, and LoadDisplayProfile deliberately does NOT --
          // its whole m_primaryOverride/StashPrimaryGlobal mechanism exists
          // so a profile's directory is BORROWED for the session and never
          // promoted to the permanent default just because it was loaded
          // (test_a_profile_load_does_not_become_the_saved_global guards
          // exactly this for the sibling fields). m_szPresetDir has already
          // been set to the profile's directory by the caller before this
          // command was queued -- both run on the render thread -- so there
          // is nothing to compare against; just rescan unconditionally.
          UpdatePresetList(true, true, false);
          size_t sep = cmd.sParam.find_last_of(L"\\/");
          std::wstring file = (sep != std::wstring::npos)
                                  ? cmd.sParam.substr(sep + 1) : cmd.sParam;
          for (int i = 0; i < (int)PresetCount(); i++) {
            if (wcscmp(PresetNameAt(i).c_str(), file.c_str()) == 0) {
              m_nCurrentPreset = i;
              break;
            }
          }
        }
        LoadPreset(cmd.sParam.c_str(), cmd.fParam);
      } else if (cmd.iParam1 >= 0 && cmd.iParam1 < m_nPresets) {
        // Index-only form: the path is built here so m_presets is read on the
        // thread that owns it (the render thread swaps it in wholesale after a
        // rescan), and re-checked against m_nPresets in case the list changed
        // between the click and this frame.
        m_nCurrentPreset = cmd.iParam1;
        wchar_t szFile[MAX_PATH];
        BuildPresetPath(cmd.iParam1, szFile, MAX_PATH);
        LoadPreset(szFile, cmd.fParam);
      }
      break;
    case RenderCmd::NavPreset:
      if (cmd.iParam1 > 0)
        NextPreset(cmd.fParam);
      else
        PrevPreset(cmd.fParam);
    ...
    case RenderCmd::CaptureScreenshot:
    {
      ...
      if (CaptureScreenshotWithFilename(filename, MAX_PATH)) {
        ...
        FormatTo(msg, L"capture/%s saved", filename);
        AddNotification(msg);
      }
      break;
    }
    case RenderCmd::RestoreAfterTesting:
      RestoreDisplaysAfterTesting();
      break;
    case RenderCmd::LoadDisplayProfile:
    {
      // g_pipeServer is already declared at file scope here; a local extern
      // would shadow it (C4459, and warnings are errors).
      const bool ok = LoadDisplayProfile(cmd.sParam.c_str());
      if (ok) wcsncpy_s(m_szStartupDisplayProfile, cmd.sParam.c_str(), _TRUNCATE);
      g_pipeServer.Send(ok ? (L"PROFILE_LOADED=" + cmd.sParam).c_str()
                           : L"PROFILE_LOADED=ERROR");
      break;
    }
    case RenderCmd::AddSpoutOutput:
    {
      ...
      if (idx == 1)
        CopyTo(newSpout.config.szName, L"MDropDX12");
      else
        swprintf(newSpout.config.szName, 128, L"MDropDX12_%d", idx);
      {
        // m_displayOutputsMutex guards just the insert, against
        // ChildConfigureNow's read on the child-worker thread. #27.
        std::lock_guard<std::recursive_mutex> lkDisp(m_displayOutputsMutex);
        m_displayOutputs.insert(m_displayOutputs.begin(), std::move(newSpout));
      }
      bSpoutChanged = true;
      RefreshDisplaysTab();
      HWND hDisp = m_displaysWindow ? m_displaysWindow->GetHWND() : NULL;
      if (hDisp) {
        HWND hList = GetDlgItem(hDisp, IDC_MW_DISP_LIST);
        if (hList) SendMessage(hList, LB_SETCURSEL, 0, 0);
      }
      UpdateDisplaysTabSelection(0);
      break;
    }
    case RenderCmd::RemoveDisplayOutput:
    {
      const int sel = cmd.iParam1;
      if (sel >= 0 && sel < (int)m_displayOutputs.size() &&
          m_displayOutputs[sel].config.type == DisplayOutputType::Spout) {
        DestroyDisplayOutput(m_displayOutputs[sel]);
        {
          // m_displayOutputsMutex guards just the erase, against
          // ChildConfigureNow's read on the child-worker thread. #27.
          std::lock_guard<std::recursive_mutex> lkDisp(m_displayOutputsMutex);
          m_displayOutputs.erase(m_displayOutputs.begin() + sel);
        }
        bSpoutChanged = true;
        EnqueueRenderCmd(RenderCmd::ResetBuffers);
        RefreshDisplaysTab();
        UpdateDisplaysTabSelection(-1);
      }
      break;
    }
    case RenderCmd::ResetDisplayOutputs:
      ResetDisplayOutputsToDefaults();
      RefreshDisplaysTab();
      UpdateDisplaysTabSelection(-1);
      break;
    case RenderCmd::IPCMessage:
      if (!cmd.sParam.empty()) {
        ...
        LaunchMessage(msgCopy.data(), cmd.iParam1 != 0);
      }
      break;
    ...
    case RenderCmd::LoadShaders:
      ...
      break;
    case RenderCmd::ApplyShaderOverride:
    {
      // Rebuild the running preset's warp and comp shaders. LoadShaders picks
      // the override's text when one is active and the preset's own when not,
      // so the same command both applies and reverts.
      ClearErrors(ERR_PRESET);
      m_activeOverride.warpFailed = false;
      m_activeOverride.compFailed = false;
    ...
    case RenderCmd::RecompileCompShader:
    {
      ...
      if (m_nMaxPSVersion > 0) {
        ...
        if (!RecompilePShader(m_pState->m_szCompShadersText, &m_shaders.comp,
          ...
          // AddRef before the assignment, matching engine.cpp:1027 and
          // engine_shaders.cpp. Without it this shared the fallback's three COM
          // pointers at a refcount of one, so the next m_shaders.comp.Clear()
          // released them and left m_fallbackShaders_ps.comp dangling for the
          // rest of the process -- reachable from Preset Editor Apply, on the
          // render thread. The other four fallback assignments all AddRef; this
          // one did not.
          ...
          m_shaders.comp.CloneFrom(m_fallbackShaders_ps.comp);
          allOK = false;
        }
    ...
    case RenderCmd::DisableAllOutputs:
      // Soft-park only — keep swap chains for the session (toggle-safe).
      {
        // m_displayOutputsMutex: out.config.bEnabled is written here and read
        // by both the ToolWindow and child-worker threads. #27.
        std::lock_guard<std::recursive_mutex> lkDisp(m_displayOutputsMutex);
        for (auto& out : m_displayOutputs) {
      ...
      RefreshDisplaysTab();
      break;
    case RenderCmd::ApplyPresetCode: {
      // The Preset Editor runs its own message pump on its own thread, so it
      // stages edited text and queues it here rather than writing CState and
      // recompiling inline -- the same rule LoadPresetPath above obeys.
      //
      // CState's code buffers are char and preset code is ASCII, so a narrow
      // copy is exact; going through WideCharToMultiByte means any stray
      // non-ASCII becomes '?' instead of silently truncating the section.
      const int need = WideCharToMultiByte(CP_ACP, 0, cmd.sParam.c_str(), -1,
                                           nullptr, 0, nullptr, nullptr);
      if (need > 0) {
        std::string narrow((size_t)need, '\0');
        WideCharToMultiByte(CP_ACP, 0, cmd.sParam.c_str(), -1,
                            &narrow[0], need, nullptr, nullptr);
        narrow.resize((size_t)need - 1);   // drop the terminator from size()
        ApplyPresetCodeSection(cmd.iParam1, cmd.iParam2, narrow.c_str(), cmd.iParam3);
      }
      break;
    }
    case RenderCmd::ApplyPresetText:
      if (!cmd.sParam.empty())
        ApplyPresetTextToState(cmd.sParam, cmd.iParam1);   // iParam1 = PresetSide
      break;
    case RenderCmd::SavePresetFile:
      if (!cmd.sParam.empty())
        SavePresetToPath(cmd.sParam.c_str());
      break;
    case RenderCmd::ReallocCanvas:
      // Render thread only. Rebuilds the feedback canvas after a canvas-limit
      // change; the new size is recomputed inside AllocateMyDX9Stuff from
      // EffectiveCanvasLimit(). Note this case MUST live here and not in
      // EngineShell::ExecuteRenderCommand -- this override has no default and
      // never chains to the base, so a case added there is dead code.
      //
      // The mirror teardown this needs lives in CleanUpMyDX9Stuff, next to the
      // ResetDynamicDescriptors call that makes it necessary — NOT here. It was
      // here first, which only covered this one caller: a window resize took
      // the same rewind with mirrors live and hung the GPU exactly the same way
      // (2026-08-23, while dragging with the mouse).
      ReallocCanvasResources();
      break;
    case RenderCmd::Quit:
