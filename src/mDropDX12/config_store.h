...
class ConfigStore {
 public:
  ...
  // Every section name the store knows, in file order. Added for the
  // migrations that move a whole family of sections out of settings.ini --
  // [*Wnd] window geometry, [DisplayOutput_*] -- which have to find the
  // sections before they can remove them, and must not carry a hardcoded list
  // that goes stale the moment a window is added or renamed.
  std::vector<std::wstring> SectionNames();

  // Every key in one section, for the same migrations SectionNames serves.
  std::vector<std::wstring> KeyNames(const wchar_t* section);
...
// Raise the shield permanently, for a process that must never write the shared
// configuration at all. A -child instance calls this at startup: parent and
// children all run from the same directory, so they all resolve the same
// settings.ini, and a child holds a deliberately reduced state -- no fallback
// texture paths, an inlined subset of the messages, one display. When such a
// child flushed, it wrote its smaller counts over the parent's, which is how a
// run came back with FallbackPaths Count=2 turned into 0 and MsgOrder 20 into
// 13. Unlike the testing shield this one cannot be lowered.
void PinConfigWriteShield();
bool IsConfigWriteShieldPinned();
