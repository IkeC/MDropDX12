...
void SpritesWindow::DoBuildControls() {
    ...
    // ListView
    {
        ...
        // Only on the FIRST build. LoadSpritesFromINI clears m_spriteEntries
        // and re-reads sprites.ini, so running it on a rebuild threw away
        // every unsaved change to position, scale, colour, blend, layer and
        // the Init/Per-Frame code -- once per mouse-move while the window was
        // being resized.
        if (IsFirstBuild()) p->LoadSpritesFromINI();
    ...
    // Button row 1: Push, Kill, Kill All, Defaults
    {
        ...
        // Same as the Shader Import Copy button: the recycle glyph pushed
        // "Defaults" out of its own button (issue 159) and the word already
        // says what the glyph was hinting at.
    ...
    // Properties: Blend + Layer
    {
        ...
        HWND hBlend = CreateCombo(hw, IDC_MW_SPR_BLENDMODE, x + propLblW, y, propComboW, 200, hFont);
        ...
        HWND hLayer = CreateCombo(hw, IDC_MW_SPR_LAYER, propCol2 + propLblW, y,
          propComboW + MulDiv(20, lineH, 26), 200, hFont);
    ...
    // Per-Frame Code
    CreateLabel(hw, L"Per-Frame", x, y, MulDiv(80, lineH, 26), lineH, hFont);
    y += lineH;
    {
        int codeH = 3 * lineH;
...
LRESULT SpritesWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
                ...
                if (SUCCEEDED(pDlg->GetResult(&pItem))) {
                    ...
                    if (SUCCEEDED(pItem->GetDisplayName(SIGDN_FILESYSPATH, &pPath))) {
                        ...
                        if (newIdx < 100000) {
                            ...
                            CopyTo(entry.szImg, relPath.c_str());
                            entry.nColorkey = 0;
    ...
    if (id == IDC_MW_SPR_PUSH && code == BN_CLICKED) {
        if (p->m_nSpriteSelected >= 0 && p->m_nSpriteSelected < (int)p->m_spriteEntries.size()) {
            ...
            p->SaveSpritesToINI();
            // Flush INI cache
            ConfigFile(p->m_szImgIniFile).Flush();
        ...
        if (SUCCEEDED(hr) && pDlg) {
            ...
            if (SUCCEEDED(pDlg->Show(hWnd))) {
                ...
                if (SUCCEEDED(pDlg->GetResult(&pItem))) {
                    ...
                    if (SUCCEEDED(pItem->GetDisplayName(SIGDN_FILESYSPATH, &pPath))) {
                        std::wstring relPath = MakeRelativeSpritePath(pPath, p->m_szContentBasePath, p->m_szMilkdrop2Path);
                        CopyTo(p->m_spriteEntries[p->m_nSpriteSelected].szImg, relPath.c_str());
