...
void PresetEditorWindow::BuildSectionList() {
  ...
  for (int i = 0; i < MAX_CUSTOM_WAVES; i++) {
    FormatTo(buf, L"Init");            add(buf, PCS_WAVE_INIT,      i, CodeLang::EEL);
    FormatTo(buf, L"Per-Frame");       add(buf, PCS_WAVE_PER_FRAME, i, CodeLang::EEL);
    FormatTo(buf, L"Per-Point");       add(buf, PCS_WAVE_PER_POINT, i, CodeLang::EEL);
  }
  ...
  for (int i = 0; i < MAX_CUSTOM_SHAPES; i++) {
    FormatTo(buf, L"Init");            add(buf, PCS_SHAPE_INIT,      i, CodeLang::EEL);
    FormatTo(buf, L"Per-Frame");       add(buf, PCS_SHAPE_PER_FRAME, i, CodeLang::EEL);
  }
}
...
void PresetEditorWindow::LoadSectionIntoEditor(int nRow) {
  ...
  if (r.section == PSEUDO_WHOLE_PRESET) {
    if (m_bTwoPresets)
      FormatTo(buf, L"Whole Preset - %s of this .milk2, code under [section] headers", szWhich);
    else
      FormatTo(buf, L"Whole Preset - the whole preset, code grouped under [section] headers");
  } else if (r.section == PSEUDO_RAW_FILE) {
    FormatTo(buf, bMilk2
    ...
    if (m_bTwoPresets) FormatTo(buf, L"%s  -  %s", r.label.c_str(), szWhich);
    else               FormatTo(buf, L"%s", r.label.c_str());
  }
  SetStatus(buf);
}
...
void PresetEditorWindow::SavePreset(bool bSaveAs) {
  ...
  if (bSaveAs) {
    ...
    if (m_pEngine->m_szCurrentPresetFile[0])
      CopyTo(szFile, m_pEngine->m_szCurrentPresetFile);
