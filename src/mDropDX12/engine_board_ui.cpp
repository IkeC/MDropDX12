...
void ButtonBoardWindow::DoBuildControls() {
    ...
    // Bank navigation row
    // In 26ths, like the tool-window layouts. Flat pixels kept this row the
    // same width while the font grew, so "Bank 1/3" and "Config" were cut off
    // inside their own controls (issue 159).
    const int lineH = GetLineHeight();
    int btnW = MulDiv(28, lineH, 26), btnH = MulDiv(22, lineH, 26);
    const int lblW = MulDiv(80, lineH, 26), cfgW = MulDiv(60, lineH, 26);
    m_hBankPrev = CreateBtn(hw, L"\x25C0", IDC_MW_BOARD_BANK_PREV, x, y, btnW, btnH, hFont);
    m_hBankLabel = CreateLabel(hw, L"Bank 1/3", x + btnW + 4, y, lblW, btnH, hFont);
    m_hBankNext = CreateBtn(hw, L"\x25B6", IDC_MW_BOARD_BANK_NEXT, x + btnW + 8 + lblW, y, btnW, btnH, hFont);
    m_hConfigBtn = CreateBtn(hw, L"Config", IDC_MW_BOARD_CONFIG, rcClient.right - 8 - cfgW, y, cfgW, btnH, hFont);
    y += btnH + 6;
...
void ButtonBoardWindow::LayoutControls() {
    ...
    int rw = rc.right - 16;
    // Must match the build above, in the same 26ths.
    const int lineH = GetLineHeight();
    int btnW = MulDiv(28, lineH, 26), btnH = MulDiv(22, lineH, 26);
    const int lblW = MulDiv(80, lineH, 26), cfgW = MulDiv(60, lineH, 26);

    MoveWindow(m_hBankPrev, x, y, btnW, btnH, TRUE);
    MoveWindow(m_hBankLabel, x + btnW + 4, y, lblW, btnH, TRUE);
    MoveWindow(m_hBankNext, x + btnW + 8 + lblW, y, btnW, btnH, TRUE);
    MoveWindow(m_hConfigBtn, rc.right - 8 - cfgW, y, cfgW, btnH, TRUE);
    y += btnH + 6;
...
HMENU ButtonBoardWindow::BuildActionSubMenu() {
    ...
    for (int cat = HKCAT_NAVIGATION; cat <= HKCAT_MISC; cat++) {
        ...
        if (cat == HKCAT_WINDOW) {
            ...
            AppendMenuW(hCatMenu, MF_STRING, 2001, L"Mirror Independent");
            AppendMenuW(hCatMenu, MF_STRING, 2002, L"Always On Top");
            itemCount += 3;
        }
...
void ButtonBoardWindow::ShowSlotContextMenu(int globalIndex, POINT screenPt) {
    ...
    else if (cmd == 2001) { // Mirror Independent
        s.action  = ButtonAction::ScriptCommand;
        s.payload = L"SIGNAL|MIRROR_INDEPENDENT";
        s.label   = L"Mirror Independent";
        m_pPanel->Invalidate();
        SaveBoard();
    }
    else if (cmd == 2002) { // Always On Top
        s.action  = ButtonAction::ScriptCommand;
        s.payload = L"SIGNAL|ALWAYS_ON_TOP";
        s.label   = L"Always On Top";
        m_pPanel->Invalidate();
        SaveBoard();
    }
