        ...
        if (!m_texture_bindings[cd.RegisterIndex].texptr &&
          ...
          if (!g_engine.GetDevice()) {
                ...
                if (GetFileAttributesW(szFilename) == 0xFFFFFFFF) {
                  ...
                  if (!siblingFound) {
                    ...
                    if (!fbFound && g_engine.m_szContentBasePath[0]) {
                      ...
                      if (GetFileAttributesW(szFilename) != 0xFFFFFFFF) fbFound = true;
                      if (!fbFound) {
                        swprintf(szFilename, L"%stextures\\%s.%s", g_engine.m_szContentBasePath, szRootName, texture_exts[z].c_str());
                        if (GetFileAttributesW(szFilename) != 0xFFFFFFFF) fbFound = true;
                      }
                    }
            ...
            while (1) {
              ...
              int nBytesCached = 0;
              int nTexCount = (int)g_engine.m_textures.size();
              for (int n = 0; n < nTexCount; n++)
                if (g_engine.m_textures[n].bEvictable && g_engine.m_textures[n].texptr) {
                  nBytesCached += g_engine.m_textures[n].nSizeInBytes;
                  nTexturesCached++;
                }
...
bool Engine::LoadShaders(PShaderSet* sh, CState* pState, bool bTick, bool bCompileOnly) {
  ...
  if (!sh->warp.ptr && !sh->warp.CT && pState->m_nWarpPSVersion > 0) {
    ...
    if (!bOK) {
      ...
      sh->warp = m_fallbackShaders_ps.warp;
    }
  ...
  if (!sh->comp.ptr && !sh->comp.CT && pState->m_nCompPSVersion > 0) {
    ...
    if (!bOK) {
      ...
      // The AddRef calls above are unchanged: they cover the three COM
      // pointers, which the compiler-generated assignment copies raw.
      sh->comp = m_fallbackShaders_ps.comp;
    }
  }
...
void Engine::CreateDX12PresetPSOs() {
  ...
  if (m_bShadertoyMode && compWantsImageFeedback &&
      compRtvFormat != DXGI_FORMAT_R32G32B32A32_FLOAT &&
      m_shaders.comp.bytecodeBlob && g_pCompVSBlob) {
    UINT dummy = 0;
    m_dx12CompFloatPSO = DX12CreatePresetPSO(
...
static void FixMilkdropRadAngShadowing(char* szShaderText) {
  if (!szShaderText || !szShaderText[0]) return;
    ...
    for (int i = 0; i < srcLen; ) {
      ...
      if (strncmp(&szShaderText[i], name, nameLen) == 0 &&
        ...
        if (*after == '(') {
          ...
          memcpy(&tmp[wi], rename, renameLen);
          wi += renameLen;
          i += nameLen;
        }
      } else {
...
static void FixSelfReferentialMacroShadow(char* szShaderText) {
  if (!szShaderText || !szShaderText[0]) return;
  ...
  for (int ni = 0; ni < (int)(sizeof(kNames) / sizeof(kNames[0])); ni++) {
    ...
    const size_t srcLen = strlen(szShaderText);
    char* out = (char*)malloc(srcLen + 32768);
    if (!out) return;
    size_t wi = 0;

    // Everything before the declared name, verbatim.
    const size_t headLen = (size_t)(decl - szShaderText);
    memcpy(out, szShaderText, headLen);
    wi = headLen;
    // The declared name -> renamed.
    memcpy(out + wi, rename, renameLen);
    wi += renameLen;
    // The initialiser, verbatim: its `aspect` must stay the macro.
    const size_t initLen = (size_t)(semi + 1 - (decl + nameLen));
    memcpy(out + wi, decl + nameLen, initLen);
    wi += initLen;
...
bool Engine::LoadShaderFromMemory(const char* szOrigShaderText, char* szFn, char* szProfile,
  ...
  */
  if ((shaderType == SHADER_WARP || shaderType == SHADER_COMP) && szProfile[0] == 'p') {
    ...
    if (!p) {
      wchar_t errMsg[512];
      swprintf(errMsg, wasabiApiLangString(IDS_ERROR_PARSING_X_X_SHADER), szProfile, szWhichShader);
      dumpmsg(errMsg, LOG_WARN);
      AddError(errMsg, 8.0f, ERR_PRESET, true);
      AutoFlagPresetError(m_szCurrentPresetFile, errMsg);
      return false;
    }
  }
  ...
  if (pShaderByteCode == NULL) {
    ...
    if (failed) {
      if (m_pShaderCompileErrors) {
        ...
        AutoFlagPresetError(m_szLoadingPreset[0] ? m_szLoadingPreset
    ...
    if (shaderType == SHADER_COMP || shaderType == SHADER_WARP) {
      ...
      char header[512];
      sprintf_s(header, "// OK: type=%s len=%d preset=%ls loading=%d\n",
  ...
  if (!compileOnly && GetDevice()) {
    ...
    if (hr != D3D_OK) {
      wchar_t errMsg[512];
      wasabiApiLangString(IDS_ERROR_CREATING_SHADER, errMsg, sizeof(errMsg));
      // dumpmsg(errMsg);
      if (bHardErrors)
        MessageBoxW(GetPluginWindow(), errMsg, wasabiApiLangString(IDS_MILKDROP_ERROR, title, 64), MB_OK | MB_SETFOREGROUND | MB_TOPMOST);
      else {
        AddError(errMsg, 6.0f, ERR_PRESET, true);
      }
      AutoFlagPresetError(m_szCurrentPresetFile, errMsg);
      return false;
    }
