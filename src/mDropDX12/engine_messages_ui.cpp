...
void MessagesWindow::DoBuildControls() {
    ...
    // Message listbox — fill available space dynamically
    {
        ...
        HWND hMsgList = CreateListBox(hw, IDC_MW_MSG_LIST, x, y, rw, listH, hFont,
    ...
    {
        HWND hLbl = CreateLabel(hw, L"Interval (s):", x, y, MulDiv(90, lineH, 26), lineH, hFont);
        if (hLbl) SetWindowLongPtr(hLbl, GWL_ID, IDC_MW_MSG_INTERVAL_LBL);
    }
    swprintf(buf, 64, L"%.1f", p->m_fMsgAutoplayInterval);
    CreateEdit(hw, buf, IDC_MW_MSG_INTERVAL, x + MulDiv(94, lineH, 26), y,
               MulDiv(60, lineH, 26), lineH, hFont, 0);
    {
        HWND hLbl = CreateLabel(hw, L"+/- (s):", x + MulDiv(170, lineH, 26), y,
                                MulDiv(60, lineH, 26), lineH, hFont);
        if (hLbl) SetWindowLongPtr(hLbl, GWL_ID, IDC_MW_MSG_JITTER_LBL);
    }
    swprintf(buf, 64, L"%.1f", p->m_fMsgAutoplayJitter);
    CreateEdit(hw, buf, IDC_MW_MSG_JITTER, x + MulDiv(234, lineH, 26), y,
               MulDiv(60, lineH, 26), lineH, hFont, 0);
    y += lineH + gap;
...
// The mode can change from anywhere -- a hotkey, IPC, the Settings combo --
// so the two checkboxes are re-read from the engine rather than assumed.
LRESULT MessagesWindow::DoMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    if (uMsg == WM_MW_MSGMODE_CHANGED) {
        SetChecked(IDC_MW_MSG_SHOW_MESSAGES, m_pEngine->MessagesEnabled());
        SetChecked(IDC_MW_MSG_SHOW_SPRITES,  m_pEngine->SpritesEnabled());
        return 0;
    }
    return -1;
}
...
LRESULT MessagesWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
    ...
    if (code == BN_CLICKED) {
        ...
        case IDC_MW_MSG_SHOW_MESSAGES:
            p->SetSpriteMessagesMode((p->m_nSpriteMessagesMode & ~1) | (bChecked ? 1 : 0));
            return 0;
        case IDC_MW_MSG_SHOW_SPRITES:
            p->SetSpriteMessagesMode((p->m_nSpriteMessagesMode & ~2) | (bChecked ? 2 : 0));
            return 0;
