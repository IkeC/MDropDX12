                                 ...
                                 const RenderContext* rc)
{
    ...
    if (m_videoFX.IsDefault() && m_pSpoutInputPSO) {
        CompositeVideoInput(isBackground, tex, srcW, srcH, rc);
        return;
    }
    ...
    // THIS surface, not the engine's canvas and not the window's client area.
    // On a mirror record both of those describe the PRIMARY, so a video
    // background was letterboxed for the main window's aspect and then
    // stretched onto a mirror of a different shape.
    if (rc && rc->canvasW > 0 && rc->canvasH > 0) {
        targetW = (float)rc->canvasW;
        targetH = (float)rc->canvasH;
...
void Engine::InitSpoutInput()
{
    ...
    if (m_szSpoutInputSender[0] != L'\0') {
        ...
        si.receiver.SetName(senderA);
    }

    if (!si.receiver.Open(m_lpDX->m_device.Get(),
                          m_lpDX->m_commandQueue.Get())) {
        DebugLogA("SpoutInput: Open failed", LOG_ERROR);
        m_spoutInput.reset();
        return;
    }
...
void Engine::UpdateSpoutInputTexture()
{
    ...
    bool received = si.receiver.Receive(&pRaw);

    // If Receive() created/replaced the resource, update our ComPtr
    ...
    if (si.receiver.IsUpdated()) {
        unsigned int w = si.receiver.SenderWidth();
        unsigned int h = si.receiver.SenderHeight();
        ...
        if (si.pReceivedTexture) {
            ...
            if (si.dx12InputTex.srvIndex == UINT_MAX) {
                ...
                CreateSRV2D(m_lpDX->m_device.Get(), si.dx12InputTex.resource.Get(), DXGI_FORMAT_B8G8R8A8_UNORM,
                           srvCpu, m_lpDX->GetSrvCpuHandleAtReadable(si.dx12InputTex.srvIndex));
...
void Engine::CompositeSpoutInput(bool isBackground)
{
    ...
    // Transition back to COPY_DEST for next frame's Receive()
...
{
    outNames = mdrop::SpoutSenderNames();
}
