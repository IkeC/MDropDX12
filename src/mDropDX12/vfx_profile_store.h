...
public:
    ...
    // resources/profiles/vfx/, one file per profile.
    std::wstring ProfileDir() const;

    // Folds a legacy resources/vfxprofiles.json into that directory,
    // once. Writes the new layout, reads it back and counts, and only
    // then removes the old file -- a crash in between costs a
    // duplicate rather than the data. Safe when there is nothing to do.
