...
void Engine::LoadSpritesFromINI() {
  ...
  DWORD len = ConfigFile(m_szImgIniFile).GetSectionNamesTo(secBuf.data(), (DWORD)secBuf.size());
  if (len == 0) return;
  ...
  for (int i : indices) {
    ...
    wchar_t img[512] = {};
    ConfigFile(m_szImgIniFile).GetStringTo(section, L"img", L"", img, 511);
    if (img[0] == 0) continue;

    SpriteEntry entry = {};
    entry.nIndex = i;
    CopyTo(entry.szImg, img);

    // Read colorkey (try colorkey_lo first for backwards compat, then colorkey)
    entry.nColorkey = (unsigned int)ConfigFile(m_szImgIniFile).GetInt(section, L"colorkey_lo", 0);
    entry.nColorkey = (unsigned int)ConfigFile(m_szImgIniFile).GetInt(section, L"colorkey", entry.nColorkey);

    // Read init_N and code_N lines (same pattern as LaunchSprite)
    char sectionA[64];
    FormatSpriteSectionA(sectionA, i);
    char szTemp[8192];
    ...
    for (int pass = 0; pass < 2; pass++) {
      ...
      for (int line = 1; ; line++) {
        ...
        ConfigFile(AutoCharFn(m_szImgIniFile)).GetStringToA(sectionA, key, "~!@#$", szTemp, 8192);
...
void Engine::SaveSpritesToINI() {
    ...
    if (len > 0) {
      ...
      while (*p) {
        if (wcsncmp(p, L"img", 3) == 0 && p[3] >= L'0' && p[3] <= L'9')
          ConfigFile(m_szImgIniFile).RemoveSection(p);
        p += wcslen(p) + 1;
      }
    }
  }
  ...
  for (auto& e : m_spriteEntries) {
    ...
    ConfigFile(m_szImgIniFile).SetString(section, L"img", e.szImg);

    if (e.nColorkey != 0) {
      wchar_t ckBuf[32];
      swprintf(ckBuf, 32, L"0x%06X", e.nColorkey);
      ConfigFile(m_szImgIniFile).SetString(section, L"colorkey", ckBuf);
    }

    // Write init_N / code_N lines
    char sectionA[64];
    FormatSpriteSectionA(sectionA, e.nIndex);
    ...
    auto writeLines = [&](const std::string& code, const char* prefix) {
      ...
      while (std::getline(ss, line)) {
        ...
        ConfigFile(AutoCharFn(m_szImgIniFile)).SetStringA(sectionA, key, line.c_str());
      }
    };
  ...
  ConfigFile(m_szImgIniFile).Flush();
}
...
bool Engine::LaunchSprite(int nSpriteNum, int nSlot) {
  ...
  img[0] = 0;
  if (nSpriteNum < 100) {
    FormatTo(section, L"img%02d", nSpriteNum);
    sprintf(sectionA, "img%02d", nSpriteNum);
  } else {
    FormatTo(section, L"img%d", nSpriteNum);
    sprintf(sectionA, "img%d", nSpriteNum);
  }

  // 1. read in image filename
  bool bBuiltinCover = false;
  ConfigFile(m_szImgIniFile).GetStringTo(section, L"img", L"", img);
  if (img[0] == 0 && nSpriteNum == 0) {
    ...
    bBuiltinCover = true;
    FormatTo(img, L"%ssprites\\cover.png", m_szMilkdrop2Path);
    ...
    wchar_t buf[1024];
    FormatResTo(buf, IDS_SPRITE_X_ERROR_COULD_NOT_FIND_IMG_OR_NOT_DEFINED, nSpriteNum);
  ...
  if (img[1] != L':')// || img[2] != '\\')
  {
    ...
    CopyTo(temp, img);
    if (m_szContentBasePath[0]) {
      FormatTo(img, L"%s%s", m_szContentBasePath, temp);
      if (GetFileAttributesW(img) == INVALID_FILE_ATTRIBUTES)
        FormatTo(img, L"%s%s", m_szMilkdrop2Path, temp);
    } else {
      FormatTo(img, L"%s%s", m_szMilkdrop2Path, temp);
    }
  }
  ...
  unsigned int ck = (unsigned int)ConfigFile(m_szImgIniFile/*GetConfigIniFile()*/).GetInt(section, L"colorkey_lo", 0x00000000);
  ck = (unsigned int)ConfigFile(m_szImgIniFile/*GetConfigIniFile()*/).GetInt(section, L"colorkey", ck);
  ...
  for (int n = 0; n < 2; n++) {
    ...
    while (!bDone) {
      ...
      ConfigFile(AutoCharFn(m_szImgIniFile)).GetStringToA(sectionA, szLineName, "~!@#$", szTemp, 8192);	// fixme
      len = lstrlenA(szTemp);
  ...
  case TEXMGR_ERR_SUCCESS:
    ...
    case TEXMGR_WARN_ERROR_IN_INIT_CODE:
      FormatResTo(buf, IDS_SPRITE_X_WARNING_ERROR_IN_INIT_CODE, nSpriteNum);
      AddError(buf, 6.0f, ERR_MISC, true);
      break;
    case TEXMGR_WARN_ERROR_IN_REG_CODE:
      FormatResTo(buf, IDS_SPRITE_X_WARNING_ERROR_IN_PER_FRAME_CODE, nSpriteNum);
      AddError(buf, 6.0f, ERR_MISC, true);
      break;
    default:
  ...
  case TEXMGR_ERR_BAD_INDEX:
    FormatResTo(buf, IDS_SPRITE_X_ERROR_BAD_SLOT_INDEX, nSpriteNum);
    AddError(buf, 6.0f, ERR_MISC, true);
    break;
    /*
  ...
  case TEXMGR_ERR_BADFILE:
    FormatResTo(buf, IDS_SPRITE_X_ERROR_IMAGE_FILE_MISSING_OR_CORRUPT, nSpriteNum);
    AddError(buf, 6.0f, ERR_MISC, true);
    break;
  case TEXMGR_ERR_OUTOFMEM:
    FormatResTo(buf, IDS_SPRITE_X_ERROR_OUT_OF_MEM, nSpriteNum);
    AddError(buf, 6.0f, ERR_MISC, true);
    break;
  }
...
void Engine::KillMilk2Sprites() {
  for (int x = 0; x < NUM_TEX; x++) {
    if (m_texmgr.m_tex[x].pSurface &&
        (m_texmgr.m_tex[x].nUserData & 0xFFFFFF00) == MILK2_SPRITE_USERDATA)
      m_texmgr.KillTex(x);
  }
}

void Engine::ApplyMilk2Sprites() {
  m_bMilk2SpritesNeedApply = false;
  KillMilk2Sprites();
  if (!SpritesEnabled() || m_milk2Sprites.empty())
    return;
  ...
  int launched = 0;
  for (const auto& spr : m_milk2Sprites) {
    if (LaunchMilk2Sprite(spr))
      launched++;
  }
  DebugLogWFmt(LOG_INFO, L"ApplyMilk2Sprites: launched %d/%d",
               launched, (int)m_milk2Sprites.size());
}

bool Engine::LaunchMilk2Sprite(const Milk2SpriteDesc& spr) {
  if (!spr.szName[0])
    return false;

  wchar_t img[MAX_PATH] = {};
  CopyTo(img, spr.szName);

  auto fileExists = [](const wchar_t* p) {
  ...
  if (img[1] != L':') {
    ...
    CopyTo(temp, img);
    const wchar_t* leaf = wcsrchr(temp, L'\\');
    if (!leaf) leaf = wcsrchr(temp, L'/');
    leaf = leaf ? leaf + 1 : temp;

    img[0] = 0;
    auto tryPath = [&](const wchar_t* base, const wchar_t* rel) {
      if (img[0] || !base || !base[0])
        return;
      wchar_t cand[MAX_PATH];
      FormatTo(cand, L"%s%s", base, rel);
      if (fileExists(cand))
        CopyTo(img, cand);
    };
    tryPath(m_szContentBasePath, temp);
    tryPath(m_szMilkdrop2Path, temp);
    if (leaf && leaf[0]) {
      wchar_t spritesRel[MAX_PATH], texturesRel[MAX_PATH];
      FormatTo(spritesRel, L"sprites\\%s", leaf);
      FormatTo(texturesRel, L"textures\\%s", leaf);
      tryPath(m_szContentBasePath, spritesRel);
      tryPath(m_szContentBasePath, texturesRel);
      tryPath(m_szMilkdrop2Path, spritesRel);
      tryPath(m_szMilkdrop2Path, texturesRel);
    }
    if (!img[0]) {
      // Fall back to content-base + original relative so LoadTex can log the miss.
      if (m_szContentBasePath[0])
        FormatTo(img, L"%s%s", m_szContentBasePath, temp);
      else
        FormatTo(img, L"%s%s", m_szMilkdrop2Path, temp);
  ...
  DebugLogWFmt(LOG_INFO, L"LaunchMilk2Sprite: %s exists=%d",
               img, fileExists(img) ? 1 : 0);

  // var_x/var_y start at MD3's centre, 0.5, and NOTHING is folded in here.
  //
  // SpriteX/SpriteY used to be added at this point, as "0.5 + spr.fX". That is
  // the wrong domain and so the wrong weight: MD3 maps x_ndc = 2*var_x - 1 and
  // then adds SpriteX to the RESULT, so folding it into var_x first doubles it
  // (2*(0.5 + fX) - 1 = 2*fX, where MD3 gives fX). It is now added in the NDC
  // domain in milkdropfs.cpp, matching MD3's own order of operations, which
  // also lets preset code assign x freely without the offset being lost.
  //
  // File SX/SY is base size; per-frame sx/sy start at 1 and multiply that size.
  // rot=0: MD3 SpriteRot=1 is a UI default, not 1 radian (that made an X).
  char initcode[8192];
  FormatToA(initcode,
            "x=%.6f;y=%.6f;"
            "sx=1;sy=1;"
            ...
            "layer=%d;"
            "burn=%.6f;"
            "repeatx=%.6f;repeaty=%.6f;",
            0.5, 0.5,
            spr.fAlpha,
            spr.nBlend,
            (spr.nLayer < 0) ? 0 : ((spr.nLayer > 4) ? 4 : spr.nLayer),
            spr.fBurn,
            spr.fRepeatX, spr.fRepeatY);
  if (!spr.szInit.empty()) {
    AppendToA(initcode, "\x01");
    AppendToA(initcode, spr.szInit.c_str());
  }

  char code[8192];
  code[0] = 0;
  if (!spr.szCode.empty()) {
    strncpy_s(code, spr.szCode.c_str(), _TRUNCATE);
    AppendToA(code, "\x01");
  }
  if (fabsf(spr.fSpeed) > 1.0e-6f) {
    char spin[80];
    FormatToA(spin, "rot=rot+%.6f/fps;", spr.fSpeed);
    AppendToA(code, spin);
  }
  ...
  int ret = m_texmgr.LoadTex(img, nSlot, initcode, code, GetTime(), GetFrame(), spr.nColorKey);
  m_texmgr.m_tex[nSlot].nUserData = MILK2_SPRITE_USERDATA | (spr.nIndex & 0xFF);
  m_texmgr.m_tex[nSlot].base_x = spr.fX;
  m_texmgr.m_tex[nSlot].base_y = spr.fY;
  m_texmgr.m_tex[nSlot].base_sx = spr.fSX;
  m_texmgr.m_tex[nSlot].base_sy = spr.fSY;

  if (ret & TEXMGR_ERROR_MASK) {
    DebugLogWFmt(LOG_WARN, L"LaunchMilk2Sprite: LoadTex failed 0x%X for %s", ret, img);
    return false;
  }
  return true;
}
