...
{
    // The DIRECTORY now, not a file. Callers show it to say where profiles
    // live, and where they live is a folder.
    //
    ...
    _snwprintf_s(out, len, _TRUNCATE, L"%s", ProfileDir().c_str());
}

std::wstring VFXProfileStore::ProfileDir() const
{
    return profiles::DirForRes(m_resourceDir, profiles::Kind::Vfx);
}


namespace {

// ── One file per profile, assembled into the store the rest of this file
//    already expects ────────────────────────────────────────────────────
//
// Everything above and below keeps working on a single JsonValue shaped
//
//     { version, <flags...>, profiles: { "<name>": {params} } }
//
// so Names/Exists/Save/Load/Delete/Import/MigrateFolder are untouched. Only
// these two functions changed: the store is now a DIRECTORY of files, one per
// profile, and they convert between it and that shape. See profile_paths.h for
// why per-file -- the short version is that a single-file store rewrites every
// profile on every save, so one bad write costs the whole set.
//
// The profile's NAME lives inside its file as "name". The filename is a
// sanitised form of it, and the two are allowed to differ: renaming must not
// have to move a file, and a name can hold characters no filesystem takes.
// Unknown members survive a rewrite here as they always did, so "name" costs
// nothing to carry.

const wchar_t* const kStoreMeta = L"_store.json";

// The leaf currently holding a profile of this name, or empty if none does.
std::wstring LeafForName(const std::wstring& dir, const std::wstring& name)
{
    for (const std::wstring& leaf : profiles::ListDir(dir)) {
        ...
        if (have.empty()) {          // written before names were stored
            have = leaf;
            if (have.size() > 5) have.resize(have.size() - 5);   // drop .json
        }
        if (_wcsicmp(have.c_str(), name.c_str()) == 0) return leaf;
    }
    return std::wstring();
}
...
bool LoadProfileStore(const VFXProfileStore* store, JsonValue& root)
{
    ...
    const std::wstring dir = store->ProfileDir();

    // Store-level flags -- version, importedFromVideoFXFolder -- were top-level
    // members of the one file. They have no profile to live in, so they get a
    // file of their own, skipped when listing profiles by the leading
    // underscore ListDir already honours.
    const JsonValue meta = JsonLoadFile((dir + kStoreMeta).c_str());
    if (meta.isObject())
        for (const auto& kv : meta.members) root.members.push_back(kv);

    JsonValue& profs = MemberRef(root, L"profiles");
    ...
    for (const std::wstring& leaf : profiles::ListDir(dir)) {
        ...
        MemberRef(profs, name.c_str()) = one;
        any = true;
    }
    return any || meta.isObject();
}
...
bool SaveProfileStore(const VFXProfileStore* store, const JsonValue& root)
{
    ...
    if (!profiles::EnsureDirPath(dir)) {
        DLOG_ERROR("VideoFX: could not create %S", dir.c_str());
        return false;
    }

    const JsonValue& profs = root[L"profiles"];

    // Write each profile to its own file, reusing the leaf that already holds
    // that name so a re-save overwrites rather than accumulating -N copies.
    bool ok = true;
    std::vector<std::wstring> written;
    for (const auto& kv : profs.members) {
        std::wstring leaf = LeafForName(dir, kv.first);
        if (leaf.empty())
            leaf = profiles::WithJson(profiles::UniqueLeafIn(dir, kv.first));

        JsonValue one = kv.second;
        MemberRef(one, L"name") = JsonValue(kv.first);   // the name travels with it

        JsonWriter w;
        w.BeginObject();
        for (const auto& m : one.members) w.Value(m.first.c_str(), m.second);
        w.EndObject();
        if (w.SaveToFile((dir + leaf).c_str())) {
            written.push_back(leaf);
        } else {
            DLOG_ERROR("VideoFX: could not write %S", (dir + leaf).c_str());
            ok = false;
        }
    }

    // A profile removed from the store is a file removed from the directory.
    // Only files this pass did not write, so a failed write above never causes
    // its own file to be deleted.
    for (const std::wstring& leaf : profiles::ListDir(dir)) {
        bool keep = false;
        for (const std::wstring& w2 : written)
            if (_wcsicmp(w2.c_str(), leaf.c_str()) == 0) { keep = true; break; }
        if (!keep && ok) DeleteFileW((dir + leaf).c_str());
    }

    // The flags, minus the profiles themselves.
    JsonWriter m;
    m.BeginObject();
    bool wroteVersion = false;
    for (const auto& kv : root.members) {
        if (_wcsicmp(kv.first.c_str(), L"profiles") == 0) continue;
        if (_wcsicmp(kv.first.c_str(), L"version") == 0) wroteVersion = true;
        m.Value(kv.first.c_str(), kv.second);
    }
    if (!wroteVersion) m.Int(L"version", 1);
    m.EndObject();
    m.SaveToFile((dir + kStoreMeta).c_str());
    return ok;
}

}  // namespace

void VFXProfileStore::MigrateSingleFile()
{
    const std::wstring legacy =
        profiles::LegacyPathRes(m_resourceDir, profiles::Kind::Vfx);
    if (legacy.empty()) return;
    if (GetFileAttributesW(legacy.c_str()) == INVALID_FILE_ATTRIBUTES) return;

    const JsonValue root = JsonLoadFile(legacy.c_str());
    if (!root.isObject()) {
        DLOG_WARN("VideoFX: %S is unreadable; leaving it alone", legacy.c_str());
        return;
    }

    // Write the new layout first and only then remove the old file, so a crash
    // between the two costs a duplicate rather than the data. Every profile has
    // to arrive: if any write fails the original stays put.
    if (!SaveProfileStore(this, root)) {
        DLOG_ERROR("VideoFX: migration to %S failed; keeping %S",
                   ProfileDir().c_str(), legacy.c_str());
        return;
    }

    // Read it back and count, rather than trusting the writes.
    JsonValue check;
    LoadProfileStore(this, check);
    const size_t before = root[L"profiles"].members.size();
    const size_t after  = check[L"profiles"].members.size();
    if (after < before) {
        DLOG_ERROR("VideoFX: migration produced %zu of %zu profiles; keeping %S",
                   after, before, legacy.c_str());
        return;
    }

    if (DeleteFileW(legacy.c_str()))
        DLOG_INFO("VideoFX: migrated %zu profile(s) to %S", after, ProfileDir().c_str());
    else
        DLOG_WARN("VideoFX: migrated %zu profile(s) but could not remove %S",
                  after, legacy.c_str());
}
...
std::wstring UniqueProfileName(const JsonValue& profiles, const std::wstring& base)
{
    ...
    for (int n = 2; n < 1000; n++) {
        ...
        FormatTo(buf, L"%s (%d)", base.c_str(), n);
...
void VFXProfileStore::MigrateFolder()
{
    ...
    FormatTo(dir, L"%svideofx\\", m_resourceDir.c_str());
    wchar_t pattern[MAX_PATH];
    FormatTo(pattern, L"%s*.json", dir);
    ...
    do {
        ...
        FormatTo(full, L"%s%s", dir, fd.cFileName);
