...
#include "format_to.h"      // FormatTo / FormatToV -- bounded, non-fatal
...
// When the last packet arrived, so the readers can tell "capture stopped" from
// "we are simply reading faster than the device delivers".
//
// This used to be a count of consecutive reads that saw pcmPos unchanged, with
// the wave blanked after 3. That is a WALL-CLOCK question asked in FRAMES, and
// the two stop agreeing the moment the frame rate passes about four times the
// device's packet rate -- roughly 400fps at a 10ms shared-mode period. Measured
// fraction of frames handed digital silence with a live device playing:
//
//     66fps      0%     a 15.2ms frame always spans a 10ms packet
//     437fps     8%
//     1000fps   60%
//     1953fps   79%     four frames of audio, then ~15 of a flat line
//
// So an uncapped instance fed itself silence on most frames, and because the
// zeros drag avg/long_avg down too, the frames that DID carry audio arrived as
// ~4.8x spikes in imm_rel -- tripping beat thresholds continuous audio would
// never have crossed. A capped-at-60 instance never saw any of it, which is why
// this survived: it only appears above 400fps.
//
// Read and written under pcmLpbMutex, which every reader and SetAudioBuf all
// hold, so no atomic is needed.
static unsigned long long pcmLastWriteMs = 0;

// How long without a packet before the wave is blanked rather than frozen.
// WASAPI shared mode delivers roughly every 10ms, so this is ~25 packets of
// slack: far too long to be tripped by a scheduling hitch at any frame rate,
// short enough that a stopped stream goes quiet promptly. The old rule was 4
// frames, which meant 67ms at 60fps and 2ms at 1953fps; this is one number at
// every rate.
static constexpr unsigned long long kAudioStaleMs = 250;

// True when no packet has arrived recently enough to trust. Callers hold the
// mutex. A zero stamp means nothing has ever been captured, which is stale by
// definition rather than by arithmetic on an unsigned wrap.
static bool AudioCaptureIsStale() {
  if (pcmLastWriteMs == 0)
    return true;
  return (GetTickCount64() - pcmLastWriteMs) > kAudioStaleMs;
}
...
void ResetAudioBuf() {
  ...
  pcmBufDrained = false;
  pcmLen = 0;
  pcmLastWriteMs = 0;   // nothing captured yet == stale, until a packet lands
}

void GetAudioBuf(unsigned char* pWaveL, unsigned char* pWaveR, int SamplesCount) {
  std::unique_lock<std::mutex> lock(pcmLpbMutex);

  // 128 is the zero level in this uint8 domain.
  const bool stale = AudioCaptureIsStale();
  if ((pcmLen < SamplesCount) || stale) {
...
void GetAudioBufFloat(float* pWaveL, float* pWaveR, int SamplesCount) {
  ...
  // Same rule as GetAudioBuf. This is the path that feeds the FFT, and through
  // it bass/mid/treb and the get_fft() texture, so a false "stale" here is what
  // turned a live spectrum into a spike train above 400fps.
  const bool stale = AudioCaptureIsStale();
  if ((pcmLen < SamplesCount) || stale) {
...
void SetAudioBuf(const BYTE* pData, const UINT32 nNumFramesToRead, const WAVEFORMATEX* pwfx, const bool bInt16) {
  ...
  // Stamped here rather than inferred from pcmPos moving: a packet of exactly
  // SAMPLE_SIZE_LPB samples leaves pcmPos unchanged (x + 576) % 576 == x, so a
  // device delivering full buffers looked identical to a dead one.
  pcmLastWriteMs = GetTickCount64();
}
...
DWORD WINAPI LoopbackCaptureThreadFunction(LPVOID pContext) {
  ...
  if (FAILED(pArgs->hr)) {
    if (pArgs && pArgs->pMDropDX12) {
      wchar_t buf[128];
      FormatTo(buf, L"CoInitialize failed: hr = 0x%08x", pArgs->hr);
...
static HRESULT LoopbackCapture(
  ...
  if (pMDropDX12 && pMMDevice) {
    try {
      ...
      if (SUCCEEDED(hrDeviceId)) {
        ...
        wchar_t buf[512];
        FormatTo(buf, L"LoopbackCapture: deviceId=%ls isRender=%ls", deviceId, bIsRenderDevice ? L"true" : L"false");
        pMDropDX12->LogDebug(buf);
      }
      else {
        wchar_t buf[256];
        FormatTo(buf, L"LoopbackCapture: GetId failed hr=0x%08x", hrDeviceId);
        pMDropDX12->LogDebug(buf);
      }
    } catch (...) {}
  }
  ...
  try {
    ...
    if (FAILED(hr)) {
      if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"IMMDevice::Activate failed: hr = 0x%08x", hr); pMDropDX12->LogInfo(buf); } catch (...) {} }
      return hr;
    }
    ...
    if (FAILED(hr)) {
      if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"IAudioClient::GetDevicePeriod failed: hr = 0x%08x", hr); pMDropDX12->LogInfo(buf); } catch (...) {} }
      return hr;
    }
    ...
    if (pMDropDX12) {
      try {
        ...
        wchar_t buf[256];
        FormatTo(buf, L"LoopbackCapture: default period %.3f ms", defaultPeriodMs);
        pMDropDX12->LogDebug(buf);
      } catch (...) {}
    }
    ...
    if (FAILED(hr)) {
      if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"IAudioClient::GetMixFormat failed: hr = 0x%08x", hr); pMDropDX12->LogInfo(buf); } catch (...) {} }
      return hr;
    }
    ...
    if (bInt16) {
      ...
      default:
        if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"Don't know how to coerce WAVEFORMATEX with wFormatTag = 0x%08x to int-16", pwfx->wFormatTag); pMDropDX12->LogInfo(buf); } catch (...) {} }
        return E_UNEXPECTED;
      }
    }
    ...
    if (NULL == hWakeUp) {
      ...
      if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"CreateWaitableTimer failed: last error = %u", dwErr); pMDropDX12->LogInfo(buf); } catch (...) {} }
    ...
    *pnFrames = 0;
    ...
    if (FAILED(hr)) {
      if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"IAudioClient::Initialize failed: hr = 0x%08x", hr); pMDropDX12->LogInfo(buf); } catch (...) {} }
      return hr;
    }
    ...
    if (FAILED(hr)) {
      if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"IAudioClient::GetService(IAudioCaptureClient) failed: hr = 0x%08x", hr); pMDropDX12->LogInfo(buf); } catch (...) {} }
      return hr;
    }
    ...
    if (!bOK) {
      DWORD dwErr = GetLastError();
      if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"SetWaitableTimer failed: last error = %u", dwErr); pMDropDX12->LogInfo(buf); } catch (...) {} }
      return HRESULT_FROM_WIN32(dwErr);
    }
    ...
    if (FAILED(hr)) {
      if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"IAudioClient::Start failed: hr = 0x%08x", hr); pMDropDX12->LogInfo(buf); } catch (...) {} }
      return hr;
    }
    ...
    for (UINT32 nPasses = 0; !bDone; nPasses++) {
      ...
      for (
        ...
        if (FAILED(hr)) {
          ...
          if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"IAudioCaptureClient::GetBuffer failed on pass %u after %u frames: hr = 0x%08x", nPasses, *pnFrames, hr); pMDropDX12->LogInfo(buf); } catch (...) {} }
          return hr;
        }

        // AUDCLNT_BUFFERFLAGS_SILENT (0x2): packet is silence — feed zeros, not an error.
        // AUDCLNT_BUFFERFLAGS_DATA_DISCONTINUITY (0x1): gap since last packet (common after
        // sleep/hibernate). The current packet data is still valid — do NOT ResetAudioBuf()
        // or we clear the ring every pass and the visualizer stutters for minutes after resume.
        const bool bSilentBuffer = (dwFlags & AUDCLNT_BUFFERFLAGS_SILENT) != 0;
        const bool bDiscontinuity = (dwFlags & AUDCLNT_BUFFERFLAGS_DATA_DISCONTINUITY) != 0;
        const DWORD dwUnknownFlags = dwFlags & ~(AUDCLNT_BUFFERFLAGS_SILENT | AUDCLNT_BUFFERFLAGS_DATA_DISCONTINUITY);

        if (bDiscontinuity && !bFirstPacket) {
          // Rate-limit: after hibernate Windows can report this on many consecutive packets.
          static UINT32 s_lastDiscLogPass = 0;
          if (nPasses == 0 || nPasses - s_lastDiscLogPass >= 500) {
            s_lastDiscLogPass = nPasses;
            if (pMDropDX12) {
              try {
                wchar_t buf[256];
                FormatTo(buf,
                  L"IAudioCaptureClient: DATA_DISCONTINUITY (flags=0x%08x) pass %u frames %u — accepting packet",
        ...
        if (0 != dwUnknownFlags) {
          ...
          if (pMDropDX12) {
            try {
              wchar_t buf[256];
              FormatTo(buf, L"IAudioCaptureClient::GetBuffer unexpected flags 0x%08x on pass %u after %u frames",
                dwFlags, nPasses, *pnFrames);
              pMDropDX12->LogInfo(buf);
            } catch (...) {}
          }
        ...
        if (0 == nNumFramesToRead) {
          bErrorInAudioData = true;
          if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"IAudioCaptureClient::GetBuffer said to read 0 frames on pass %u after %u frames", nPasses, *pnFrames); pMDropDX12->LogInfo(buf); } catch (...) {} }
        }
        ...
        if (FAILED(hr)) {
          if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"IAudioCaptureClient::ReleaseBuffer failed on pass %u after %u frames: hr = 0x%08x", nPasses, *pnFrames, hr); pMDropDX12->LogInfo(buf); } catch (...) {} }
          return hr;
        }
      ...
      if (FAILED(hr)) {
        if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"IAudioCaptureClient::GetNextPacketSize failed on pass %u after %u frames: hr = 0x%08x", nPasses, *pnFrames, hr); pMDropDX12->LogInfo(buf); } catch (...) {} }
        return hr;
      }
      ...
      if (WAIT_OBJECT_0 == dwWaitResult) {
        if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"Received stop event after %u passes and %u frames", nPasses, *pnFrames); pMDropDX12->LogInfo(buf); } catch (...) {} }
        bDone = true;
        continue;
      }

      if (WAIT_OBJECT_0 + 1 != dwWaitResult) {
        if (pMDropDX12) { try { wchar_t buf[256]; FormatTo(buf, L"Unexpected WaitForMultipleObjects return value %u on pass %u after %u frames", dwWaitResult, nPasses, *pnFrames); pMDropDX12->LogInfo(buf); } catch (...) {} }
        return E_UNEXPECTED;
      }
    } // capture loop
