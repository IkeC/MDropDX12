...
void Engine::LoadDisplayOutputSettings()
{
    ...
    int count = Config().GetInt(L"DisplayOutputs", L"Count", -1);
    int legacyOpacity = Config().GetInt(L"DisplayOutputs", L"MirrorOpacity", 100);
    if (legacyOpacity < 1) legacyOpacity = 1;
    if (legacyOpacity > 100) legacyOpacity = 100;
    m_bMirrorModeForAltS = Config().GetBool(L"DisplayOutputs", L"MirrorModeForAltS", false);
    m_bMirrorPromptDisabled = Config().GetBool(L"DisplayOutputs", L"MirrorPromptDisabled", false);
    m_bMirrorIndependentDefault = Config().GetBool(L"DisplayOutputs", L"MirrorIndependentDefault", false);
    {
        int fps = Config().GetInt(L"DisplayOutputs", L"MirrorMaxFps", 0);
        if (fps < 0) fps = 0;
    ...
    for (int i = 0; i < count; i++) {
        ...
        wchar_t typeBuf[32] = {};
        Config().GetStringTo(section, L"Type", L"Spout", typeBuf, 32);
        ...
        out.config.bEnabled = Config().GetBool(section, L"Enabled", false);

        wchar_t nameBuf[128] = {};
        Config().GetStringTo(section, L"Name", L"MDropDX12", nameBuf, 128);
        wcsncpy_s(out.config.szName, nameBuf, _TRUNCATE);

        if (out.config.type == DisplayOutputType::Monitor) {
            wchar_t devBuf[32] = {};
            Config().GetStringTo(section, L"DeviceName", L"", devBuf, 32);
            wcsncpy_s(out.config.szDeviceName, devBuf, _TRUNCATE);
            out.config.bFullscreen = Config().GetBool(section, L"Fullscreen", true);
            out.config.nOpacity = Config().GetInt(section, L"Opacity", 100);
            if (out.config.nOpacity < 1) out.config.nOpacity = 1;
            if (out.config.nOpacity > 100) out.config.nOpacity = 100;
            out.config.bClickThrough = Config().GetBool(section, L"ClickThrough", false);
            out.config.bIndependentRender = Config().GetBool(section, L"IndependentRender", m_bMirrorIndependentDefault);
        }
        else {
            out.config.bFixedSize = Config().GetBool(section, L"FixedSize", false);
            out.config.nWidth = Config().GetInt(section, L"Width", 1920);
            out.config.nHeight = Config().GetInt(section, L"Height", 1080);
        }
...
void Engine::SaveDisplayOutputSettings()
{
    ...
    Config().SetString(L"DisplayOutputs", L"Count", buf);
    swprintf(buf, 64, L"%d", m_bMirrorModeForAltS ? 1 : 0);
    Config().SetString(L"DisplayOutputs", L"MirrorModeForAltS", buf);
    swprintf(buf, 64, L"%d", m_bMirrorPromptDisabled ? 1 : 0);
    Config().SetString(L"DisplayOutputs", L"MirrorPromptDisabled", buf);
    swprintf(buf, 64, L"%d", m_bMirrorIndependentDefault ? 1 : 0);
    Config().SetString(L"DisplayOutputs", L"MirrorIndependentDefault", buf);
    swprintf(buf, 64, L"%d", m_nMirrorMaxFps.load());
    Config().SetString(L"DisplayOutputs", L"MirrorMaxFps", buf);
    ...
    for (int i = 0; i < count; i++) {
        ...
        Config().SetString(section, L"Type", cfg.type == DisplayOutputType::Monitor ? L"Monitor" : L"Spout");
        swprintf(buf, 64, L"%d", cfg.bEnabled ? 1 : 0);
        Config().SetString(section, L"Enabled", buf);
        Config().SetString(section, L"Name", cfg.szName);

        if (cfg.type == DisplayOutputType::Monitor) {
            Config().SetString(section, L"DeviceName", cfg.szDeviceName);
            swprintf(buf, 64, L"%d", cfg.bFullscreen ? 1 : 0);
            Config().SetString(section, L"Fullscreen", buf);
            swprintf(buf, 64, L"%d", cfg.nOpacity);
            Config().SetString(section, L"Opacity", buf);
            swprintf(buf, 64, L"%d", cfg.bClickThrough ? 1 : 0);
            Config().SetString(section, L"ClickThrough", buf);
            swprintf(buf, 64, L"%d", cfg.bIndependentRender ? 1 : 0);
            Config().SetString(section, L"IndependentRender", buf);
        }
        else {
            swprintf(buf, 64, L"%d", cfg.bFixedSize ? 1 : 0);
            Config().SetString(section, L"FixedSize", buf);
            swprintf(buf, 64, L"%d", cfg.nWidth);
            Config().SetString(section, L"Width", buf);
            swprintf(buf, 64, L"%d", cfg.nHeight);
            Config().SetString(section, L"Height", buf);
        }
    }
...
{
    if (out.config.type == DisplayOutputType::Spout) {
        // A -child instance publishes no Spout sender. Sender names live in a
        // namespace shared across the whole machine, and a child reading the
        // same settings.ini would advertise a second sender under the parent's
        // name -- receivers would then bind to whichever registered last. The
        // parent owns the outward-facing sender; children only render.
        if (m_bChildMode)
            return;
...
void Engine::StartMirrorThread()
{
    ...
    m_mirrorWorkList->SetName(L"MirrorWorkList");   // DRED attribution
...
void Engine::MirrorThreadDrawAndPresent()
{
    ...
    if (!m_lpDX || !m_lpDX->m_device || !m_mirrorQueue || !m_mirrorWorkList)
        return;
    // Jerkiness instrumentation (worker-thread only; published once a second).
    ...
    if (maxFps > 0) {
        ...
        if (pf == m_nWorkerSeenPrimaryFrame)
            return; // primary has not advanced — nothing new to keep up with
        m_nWorkerSeenPrimaryFrame = pf;
    }
    ...
    if (m_orientLastWrite >= 0) {
        ...
        m_orientPublishedSeq.fetch_add(1, std::memory_order_release);
...
void Engine::SendToDisplayOutputs()
{
    ...
    // A large gap between them means panels are re-showing the same orient frame.
    {
        ...
        if (m_bMirrorsActive && m_hMirrorWake)
            SetEvent(m_hMirrorWake);
        LARGE_INTEGER pNow;
    ...
    // Latch the current preset's canvas limit ONLY while no pipe exists, then
    // clamp with it. Adopting a new limit under a live pipe would change the
    // computed size and trigger a recreate, which hangs the GPU (see
    // m_nOrientCanvasLimit). Render thread only.
    if (!m_orientPipe.ready)
        m_nOrientCanvasLimit = m_nCanvasLimitApplied;
    // Same clamp EnsureOrientPipeline applies, or we recreate+WaitForGpu every
    // frame and TDR. One function, two call sites — never two copies.
    if (oppW > 0 && oppH > 0)
        ClampOrientCanvas(oppW, oppH);
    ...
    if (anyOppositeIndep && oppW > 0 && oppH > 0 && workerIdle) {
        ...
        // Mode is part of "need": the worker is barred from rebuilding, so if
        // this test misses a reason the pipe is unusable, nobody rebuilds it
        // and the mirrors freeze. OrientPipeReadyForMode covers ready-ness and
        // the per-mode texture set together.
        const bool needPipe = !OrientPipeReadyForMode() ||
    ...
    for (auto& out : m_displayOutputs) {
        ...
        if (useOrient && ms.bEverPresented &&
            ms.lastPubSeq == m_orientPublishedSeq.load(std::memory_order_acquire)) {
            ms.bFreshDraw = false;
            continue;
        }
