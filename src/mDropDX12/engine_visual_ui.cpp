...
void VisualWindow::DoBuildControls() {
  ...
  // The percentage readout and the space reserved for it are the SAME
  // number. They were 50-in-26ths and a flat 60, so at a large font the
  // readout grew and the gap did not, and it hung past the client edge.
  const int pctW = MulDiv(50, lineH, 26);
  int sliderW = rw - lw - pctW - 12;
  wchar_t buf[64];
  ...
  TrackAnchored(CreateSlider(hw, IDC_MW_OPACITY, x + lw + 4, y, sliderW, lineH, 0, 100, (int)(p->fOpacity * 100)), kAnchorStretchWide);
  swprintf(buf, 64, L"%d%%", (int)(p->fOpacity * 100));
  {
    HWND hLbl = CreateWindowExW(0, L"STATIC", buf, WS_CHILD | WS_VISIBLE | SS_LEFT,
      x + lw + sliderW + 8, y, pctW, lineH, hw,
      (HMENU)(INT_PTR)IDC_MW_OPACITY_LABEL, GetModuleHandle(NULL), NULL);
    if (hLbl && hFont) SendMessage(hLbl, WM_SETFONT, (WPARAM)hFont, TRUE);
    TrackAnchored(hLbl, kAnchorTopRight);
  }
  y += lineH + gap;

  // Render Quality
  TrackControl(CreateLabel(hw, L"Render Quality:", x, y, lw, lineH, hFont));
  TrackAnchored(CreateSlider(hw, IDC_MW_RENDER_QUALITY, x + lw + 4, y, sliderW, lineH, 0, 100, (int)(p->m_fRenderQuality * 100)), kAnchorStretchWide);
  ...
  TrackAnchored(CreateSlider(hw, IDC_MW_MESH_SIZE, x + lw + 4, y, sliderW, lineH, 1, 24, p->m_nGridX / 8), kAnchorStretchWide);
  ...
  {
    HWND hTexBits = CreateCombo(hw, IDC_MW_TEX_BITS, x + lw + 4, y, 120, lineH * 6, hFont);
    const wchar_t* texLabels[] = { L"8-bit", L"16-bit float", L"32-bit float" };
    const int texValues[] = { 8, 16, 32 };
    int selIdx = 0;
    for (int i = 0; i < 3; i++) {
      SendMessageW(hTexBits, CB_ADDSTRING, 0, (LPARAM)texLabels[i]);
      if (texValues[i] == p->m_nTexBitsPerCh) selIdx = i;
    }
    SendMessage(hTexBits, CB_SETCURSEL, selIdx, 0);
    TrackControl(hTexBits);
  }
  y += lineH + gap;
  ...
  // Numeric fields with trailing hints: size the hint to the remaining client width
  // so large fonts / narrow windows don't clip "(0 = unlimited)" etc.
  auto addLabeledEditWithHint = [&](const wchar_t* label, int editId, const wchar_t* editText,
                                    const wchar_t* hint) {
    const int editW = MulDiv(56, lineH, 26);
    const int editX = x + lw + 4;
    const int hintX = editX + editW + 8;
    const int contentRight = x + rw;
    int hintW = contentRight - hintX;
    if (hintW < 24) hintW = 24;

    TrackControl(CreateLabel(hw, label, x, y, lw, lineH, hFont));
    TrackControl(CreateEdit(hw, editText, editId, editX, y, editW, lineH, hFont));
    TrackControl(CreateLabel(hw, hint, hintX, y, hintW, lineH, hFont));
    y += lineH + gap;
  };

  swprintf(buf, 64, L"%d", p->m_nMaxShapeInstances);
  addLabeledEditWithHint(L"Max Instances:", IDC_MW_GPU_MAX_INST, buf, L"(0 = unlimited)");
  ...
  TrackControl(CreateEdit(hw, buf, IDC_MW_GPU_SCALE_BASE, x + lw + 4, y, MulDiv(56, lineH, 26), lineH, hFont));
  y += lineH + gap;
  ...
  TrackControl(CreateEdit(hw, buf, IDC_MW_GPU_HEAVY_THRESHOLD, x + lw + 4, y, MulDiv(56, lineH, 26), lineH, hFont));
  y += lineH + gap;

  swprintf(buf, 64, L"%d", p->m_nMinAvailableVramPercent);
  // Shorter left label so the "% of budget (0 = off)" hint has room at large font sizes
  addLabeledEditWithHint(L"Min free VRAM %:", IDC_MW_GPU_MIN_VRAM_PCT, buf, L"of budget (0 = off)");
  y += 4;
  ...
  {
    HWND hFpsCap = CreateCombo(hw, IDC_MW_FPS_CAP, x + lw + 4, y, 120, lineH * 8, hFont);
    int selIdx = FpsCapFindIndex(p->m_max_fps_fs);
    if (selIdx < 0) selIdx = FpsCapFindIndex(40);
    for (int i = 0; i < kFpsCapOptionCount; i++)
      SendMessageW(hFpsCap, CB_ADDSTRING, 0, (LPARAM)kFpsCapOptions[i].uiLabel);
    SendMessage(hFpsCap, CB_SETCURSEL, selIdx, 0);
    TrackControl(hFpsCap);
  }
  y += lineH + gap + 4;
...
LRESULT VisualWindow::DoHScroll(HWND hWnd, int id, int pos) {
  ...
  case IDC_MW_MESH_SIZE: {
    ...
    // The render thread owns m_nGridX/m_nGridY -- see RenderCmd::SetMeshSize.
    // The label below uses the local, not the member, so the UI still tracks
    // the drag even though the engine adopts the value a frame later.
    m_pEngine->m_reqGridX.store(gridX);
    m_pEngine->m_reqGridY.store(gridX * 3 / 4);
    m_pEngine->EnqueueRenderCmd(RenderCmd::SetMeshSize);
...
LRESULT VisualWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
  ...
  if (id == IDC_MW_GPU_RELOAD_PRESET && code == BN_CLICKED) {
    // NOT p->NextPreset() -- this runs on the tool window's own thread, and
    // Engine::NextPreset/LoadPreset race the render thread's own handling of
    // m_presetLoadThread from here (see tool_window.h's RequestLoadPreset
    // comment: two threads touching one std::thread aborts the process with
    // WER 0xC0000409, unrecoverable and unloggable). RequestNavPreset
    // marshals this onto the render thread instead, where NextPreset is
    // actually safe to call from.
    RequestNavPreset(1, 0.0f);
    return 0;
  }
  ...
  if (id == IDC_MW_RESET_VISUAL && code == BN_CLICKED) {
    ...
    // Reset mesh size
    p->m_reqGridX.store(64);
    p->m_reqGridY.store(48);
    p->EnqueueRenderCmd(RenderCmd::SetMeshSize);
    ...
    p->m_nMinAvailableVramPercent = 5;
    ...
    SetWindowTextW(GetDlgItem(hWnd, IDC_MW_GPU_MIN_VRAM_PCT), L"5");
  ...
  if (code == EN_KILLFOCUS) {
    ...
    case IDC_MW_GPU_HEAVY_THRESHOLD:
      ...
      return 0;
    case IDC_MW_GPU_MIN_VRAM_PCT:
      p->m_nMinAvailableVramPercent = _wtoi(buf);
      if (p->m_nMinAvailableVramPercent < 0) p->m_nMinAvailableVramPercent = 0;
      if (p->m_nMinAvailableVramPercent > 90) p->m_nMinAvailableVramPercent = 90;
      return 0;
    }
  }
