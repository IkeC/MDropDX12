...
void Engine::SaveHotkeySettings()
{
    ...
    Config().SetString(L"Hotkeys", L"Version", L"4");
    ...
    for (int i = 0; i < NUM_HOTKEYS; i++) {
        ...
        Config().SetString(L"Hotkeys", modKey, buf);
        swprintf(buf, 64, L"%u", m_hotkeys[i].vk);
        Config().SetString(L"Hotkeys", vkKey, buf);
        swprintf(buf, 64, L"%d", (int)m_hotkeys[i].scope);
        Config().SetString(L"Hotkeys", scopeKey, buf);
        ...
        Config().SetString(L"Hotkeys", gModKey, buf);
        swprintf(buf, 64, L"%u", m_hotkeys[i].globalVK);
        Config().SetString(L"Hotkeys", gVkKey, buf);
    }

    // Save dynamic user hotkeys
    int count = (int)m_userHotkeys.size();
    swprintf(buf, 64, L"%d", count);
    Config().SetString(L"Hotkeys", L"UserHotkey_Count", buf);
    swprintf(buf, 64, L"%d", m_nextUserHotkeyId);
    Config().SetString(L"Hotkeys", L"UserHotkey_NextID", buf);
    ...
    for (int i = 0; i < count; i++) {
        ...
        Config().SetString(L"Hotkeys", keyBuf, buf);
        ...
        swprintf(keyBuf, 128, L"%sLabel", prefix);
        Config().SetString(L"Hotkeys", keyBuf, uh.label.c_str());

        swprintf(keyBuf, 128, L"%sCmd", prefix);
        Config().SetString(L"Hotkeys", keyBuf, uh.command.c_str());
    }
    ...
    for (int i = count; i < count + 20; i++) {
        ...
        wchar_t test[16];
        Config().GetStringTo(L"Hotkeys", keyBuf, L"", test, 16);
        ...
        for (auto* suffix : { L"ID", L"Type", L"Mod", L"VK", L"Scope", L"Label", L"Cmd" }) {
            ...
            Config().Remove(L"Hotkeys", keyBuf);
        }
    }
}
...
{
    if (!hwnd) return;
    // A -child instance registers nothing system-wide. RegisterHotKey is
    // first-come-first-served per key combination, so a child would either
    // fail (the parent already holds the key) or, if it started first, steal
    // every global binding and act on it while the parent stayed silent.
    // Children are driven over the named pipe and take no direct input.
    if (m_bChildMode) return;
    for (int i = 0; i < NUM_HOTKEYS; i++) {
