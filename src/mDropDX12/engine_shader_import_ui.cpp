...
// The four tables above are indexed by ChannelSource and MUST stay the same
// length as the enum (issue 19). Nothing else ties them together: the enum
// lives in tool_window.h and the tables live here, which is a gap that file
// splitting created. Adding a channel without extending all four reads past
// the end of whichever one was missed -- silently, since these are file-scope
// arrays, not containers.
static_assert(_countof(kChannelSamplers)    == CHAN_COUNT, "kChannelSamplers is out of step with ChannelSource");
static_assert(_countof(kChannelSamplers_ST) == CHAN_COUNT, "kChannelSamplers_ST is out of step with ChannelSource");
static_assert(_countof(kChannelNames)       == CHAN_COUNT, "kChannelNames is out of step with ChannelSource");
static_assert(_countof(kChannelTexDim)      == CHAN_COUNT, "kChannelTexDim is out of step with ChannelSource");
...
LRESULT WelcomeWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
    ...
    case IDC_MW_WELCOME_BROWSE:
    {
        ...
        if (BrowseForFolder(m_hWnd, L"Select folder containing presets and textures", folder, MAX_PATH)) {
            ...
            if (hasPresetSubdir)
                CopyTo(p->m_szPresetDir, presetDir);
            else
                CopyTo(p->m_szPresetDir, folder);

            // Save to INI so it persists
            Config().SetString(L"Settings", L"szPresetDir", p->m_szPresetDir);
...
std::string ShaderEditorWindow::GetGLSL() {
    if (!m_hWnd) return "";
    HWND hGlslEdit = GetDlgItem(m_hWnd, IDC_MW_SHEDITOR_GLSL_EDIT);
    int len = GetWindowTextLengthW(hGlslEdit);
    if (len <= 0) return "";
    std::wstring w(len + 1, L'\0');
    GetWindowTextW(hGlslEdit, w.data(), len + 1);
    std::string s;
    s.reserve(len);
    for (int i = 0; i < len; i++) {
        wchar_t ch = w[i];
...
std::string ShaderEditorWindow::GetHLSL() {
    if (!m_hWnd) return "";
    HWND hHlslEdit = GetDlgItem(m_hWnd, IDC_MW_SHEDITOR_HLSL_EDIT);
    int len = GetWindowTextLengthW(hHlslEdit);
    if (len <= 0) return "";
    std::wstring w(len + 1, L'\0');
    GetWindowTextW(hHlslEdit, w.data(), len + 1);
...
std::string ShaderEditorWindow::GetNotes() {
    if (!m_hWnd) return "";
    HWND hNotesEdit = GetDlgItem(m_hWnd, IDC_MW_SHEDITOR_NOTES_EDIT);
    int len = GetWindowTextLengthW(hNotesEdit);
    if (len <= 0) return "";
    std::wstring w(len + 1, L'\0');
    GetWindowTextW(hNotesEdit, w.data(), len + 1);
    std::string s;
    s.reserve(len);
    for (int i = 0; i < len; i++) {
        wchar_t ch = w[i];
...
LRESULT ShaderEditorWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
    if (code == BN_CLICKED) {
        ...
        case IDC_MW_SHEDITOR_COPY: {
            HWND hHlslEdit = GetDlgItem(hWnd, IDC_MW_SHEDITOR_HLSL_EDIT);
            int len = GetWindowTextLengthW(hHlslEdit);
            if (len > 0) {
                std::wstring text(len + 1, L'\0');
                GetWindowTextW(hHlslEdit, text.data(), len + 1);
                if (OpenClipboard(hWnd)) {
                    EmptyClipboard();
...
void ShaderImportWindow::DoBuildControls() {
    ...
    HWND hList = CreateListBox(hw, IDC_MW_SHIMPORT_PASS_LIST, x, y, rw, listH, hFont,
    ...
    // Channel input combos (2x2 grid): ch0/ch1 on row 1, ch2/ch3 on row 2
    {
        ...
        for (int row = 0; row < 2; row++) {
            for (int col = 0; col < 2; col++) {
                ...
                HWND hChanCombo = CreateCombo(hw, IDC_MW_SHIMPORT_CH0 + ch,
                    cx + lblW, y, comboW, lineH * 8, hFont);
                for (int s = 0; s < CHAN_COUNT; s++)
                    SendMessageW(hChanCombo, CB_ADDSTRING, 0, (LPARAM)kChannelNames[s]);
                TrackControl(hChanCombo);
            }
            y += lineH + gap;
        }
    }
        ...
        x, y, 150, lineH, hw, NULL, NULL, NULL));
    {
        ...
        // The scissors glyph cost more width than it earned: it pushed "Copy" out
        // of its own button (issue 159), and the word alone is not ambiguous.
        TrackControl(CreateBtn(hw, L"Copy", IDC_MW_SHIMPORT_COPY_STATUS,
            x + MulDiv(150, lineH, 26) + 4, y, copyW, lineH, hFont));
    }
    y += lineH + gap;
...
void ShaderImportWindow::SyncChannelCombos() {
    ...
    for (int ch = 0; ch < 4; ch++) {
        HWND hChanCombo = GetDlgItem(m_hWnd, IDC_MW_SHIMPORT_CH0 + ch);
        if (hChanCombo)
            SendMessage(hChanCombo, CB_SETCURSEL, m_passes[m_nSelectedPass].channels[ch], 0);
    }
}
...
LRESULT ShaderImportWindow::DoCommand(HWND hWnd, int id, int code, LPARAM lParam) {
    ...
    if (id >= IDC_MW_SHIMPORT_CH0 && id <= IDC_MW_SHIMPORT_CH3 && code == CBN_SELCHANGE) {
        ...
        HWND hChanCombo = GetDlgItem(hWnd, id);
        int sel = (int)SendMessage(hChanCombo, CB_GETCURSEL, 0, 0);
        if (sel >= 0 && sel < CHAN_COUNT &&
            ...
            if (sel == CHAN_TEXTURE_FILE) {
                ...
                if (GetOpenFileNameW(&ofn)) {
                    ...
                    SendMessage(hChanCombo, CB_SETCURSEL, prev, 0);
                }
            } else {
    ...
    if (code == BN_CLICKED) {
        ...
        case IDC_MW_SHIMPORT_COPY_STATUS: {
            ...
            HWND hStatusEdit = GetDlgItem(hWnd, IDC_MW_SHIMPORT_ERROR_EDIT);
            if (hStatusEdit) {
                int len = GetWindowTextLengthW(hStatusEdit);
                if (len > 0) {
                    std::wstring txt(len + 1, L'\0');
                    GetWindowTextW(hStatusEdit, &txt[0], len + 1);
                    txt.resize(len);
                    if (OpenClipboard(hWnd)) {
                        EmptyClipboard();
                        ...
                        if (hMem) {
                            ...
                            wcsncpy_s(p, txt.size() + 1, txt.c_str(), _TRUNCATE);
                            GlobalUnlock(hMem);
                            SetClipboardData(CF_UNICODETEXT, hMem);
                        }
                        CloseClipboard();
    ...
    if (msg == WM_TIMER && wParam == 1) {
        ...
        if (compOK && bufAOK && bufBOK && bufCOK && bufDOK) {
            ...
            for (int di = 0; di < diagCount; di++) {
                ...
                wchar_t diagName[64];
                FormatTo(diagName, L"diag_%ls_shader_error.txt", diagNames[di]);
...
void ShaderImportWindow::AnalyzeChannels(ShaderPass& pass, bool jsonLoaded) {
    ...
    for (int ch = 0; ch < 4; ch++) {
        char chName[16];
        FormatToA(chName, "iChannel%d", ch);
        ...
        {
            char pat[64];
            FormatToA(pat, "texelFetch(iChannel%d", ch);
            bool isAudio = false;
            size_t pos = 0;
            ...
            if (isAudio) {
                ...
                char lodCheck[64];
                FormatToA(lodCheck, "textureLod(iChannel%d", ch);
                char texCheck[64];
                FormatToA(texCheck, "texture(iChannel%d", ch);
        ...
        {
            char pat[64];
            FormatToA(pat, "texture(iChannel%d", ch);
            bool found3D = false;
            size_t pos = 0;
            while ((pos = src.find(pat, pos)) != std::string::npos) {
        ...
        if (isBufferB && ch == 0) {
            ...
            char fetchPat[64];
            FormatToA(fetchPat, "texelFetch(iChannel%d", ch);
        ...
        if (isBufferA) {
            char lodPat[64];
            FormatToA(lodPat, "textureLod(iChannel%d,", ch);
...
void ShaderImportWindow::ConvertAndApply() {
    ...
    // Diagnostic: log channel assignments after analysis
    {
        ...
        for (auto& p : m_passes) {
            ...
            FormatToA(line, "  %s: ch0=%s ch1=%s ch2=%s ch3=%s (glsl=%d chars)\n",
                name8,
    ...
    if (isMilk3) {
        ...
        // Write notes (if any pass has notes)
        {
            ...
            if (hasNotes) {
                ...
                for (auto& pass : m_passes) {
                    ...
                    if (pass.name == L"Image") w.String(kMilk3KeyMain, wn.c_str());
                    else if (pass.name == L"Buffer A") w.String(Milk3PassKey(0), wn.c_str());
                    else if (pass.name == L"Buffer B") w.String(Milk3PassKey(1), wn.c_str());
                    else if (pass.name == L"Buffer C") w.String(Milk3PassKey(2), wn.c_str());
                    else if (pass.name == L"Buffer D") w.String(Milk3PassKey(3), wn.c_str());
                }
                w.EndObject();
            }
        }
        ...
        for (auto& pass : m_passes) {
            if (pass.name == L"Buffer A" && !pass.hlslOutput.empty())
                w.String(Milk3PassKey(0), hlslToWide(pass.hlslOutput).c_str());
            else if (pass.name == L"Buffer B" && !pass.hlslOutput.empty())
                w.String(Milk3PassKey(1), hlslToWide(pass.hlslOutput).c_str());
            else if (pass.name == L"Buffer C" && !pass.hlslOutput.empty())
                w.String(Milk3PassKey(2), hlslToWide(pass.hlslOutput).c_str());
            else if (pass.name == L"Buffer D" && !pass.hlslOutput.empty())
                w.String(Milk3PassKey(3), hlslToWide(pass.hlslOutput).c_str());
            else if (pass.name == L"Image" && !pass.hlslOutput.empty())
                w.String(kMilk3KeyMain, hlslToWide(pass.hlslOutput).c_str());
        }

        // Channel mappings
        w.BeginObject(kMilk3KeyInputs);
        const bool hasPass[4] = { hasBufferA, hasBufferB, hasBufferC, hasBufferD };
        for (int pi = 0; pi < 4; pi++) {
            if (!hasPass[pi]) continue;
            w.BeginObject(Milk3PassKey(pi));
            w.String(kMilk3KeyChannel0, L"self");
            w.EndObject();
        }
        w.BeginObject(kMilk3KeyMain);
        w.String(kMilk3KeyChannel0, hasBufferA ? Milk3PassKey(0) : L"self");
        w.EndObject();
        w.EndObject(); // inputs
        w.EndObject(); // root
    ...
    try {
        ...
        auto findMulAssign = [&](size_t& outIdx, size_t& outSkip) -> bool {
            for (int n = 2; n <= 4; n++) {
                for (int m = 2; m <= 4; m++) {
                    char tokens[2][20];
                    if (n == m) FormatToA(tokens[0], "*=mat%d(", n);
                    else FormatToA(tokens[0], "*=mat%dx%d(", n, m);
                    FormatToA(tokens[1], "*=float%dx%d(", n, m);
                    for (int ti = 0; ti < 2; ti++) {
                        size_t idx = result.find(tokens[ti]);
                        if (idx != std::string::npos) {
                            outIdx = idx;
                            outSkip = strlen(tokens[ti]);
                            char mt[16]; FormatToA(mt, "float%dx%d", n, m);
                            matType = mt;
                            return true;
                        }
                    }
        ...
        // Handles both square (mat2, mat3, mat4) and non-square (mat2x3, mat4x2, etc.)
        {
            ...
            for (int n = 2; n <= 4 && !handled; n++) {
                for (int m = 2; m <= 4 && !handled; m++) {
                    char matTokens[3][20];
                    int numTokens = 0;
                    if (n == m) { FormatToA(matTokens[numTokens++], "mat%d(", n); }
                    else { FormatToA(matTokens[numTokens++], "mat%dx%d(", n, m); }
                    FormatToA(matTokens[numTokens++], "float%dx%d(", n, m);
        ...
        for (int n = 2; n <= 4; n++) {
            for (int m = 2; m <= 4; m++) {
                ...
                FormatToA(from1, "mat%dx%d(", n, m);
                FormatToA(to1, "float%dx%d(", n, m);
                FormatToA(from2, "mat%dx%d ", n, m);
                FormatToA(to2, "float%dx%d ", n, m);
...
void ShaderImportWindow::ConvertGLSLtoHLSL(int passOverride) {
    ...
    try {
        ...
        for (int ci = 0; ci < 4; ci++) {
            char from[32];
            FormatToA(from, "iChannelResolution[%d]", ci);
        ...
        // iChannel samplers — use per-pass channel configuration
        {
            ...
            for (int i = 0; i < 4; i++) {
                char from[16], to[32];
                FormatToA(from, "iChannel%d", i);
                if (ch[i] == CHAN_TEXTURE_FILE) {
                    FormatToA(to, "sampler_chtex%d", i);
                } else {
                    int src = (ch[i] >= 0 && ch[i] < CHAN_COUNT) ? ch[i] : 0;
                    CopyToA(to, kChannelSamplers_ST[src]);
                }
        ...
        if (m_passes[passIdx].name == L"Image") {
            ...
            for (auto* s : extSamplers) {
                char from[64], to[64];
                FormatToA(from, "tex2D_lod0(%s,", s);
                FormatToA(to, "tex2D_flipV(%s,", s);
                replaceAll(inp, from, to);
                FormatToA(from, "tex2Dlod_conv(%s,", s);
                FormatToA(to, "tex2Dlod_flipV(%s,", s);
                replaceAll(inp, from, to);
            }
        }
        ...
        // Declare custom channel texture samplers if used
        {
            ...
            for (int i = 0; i < 4; i++) {
                char sampName[32];
                FormatToA(sampName, "sampler_chtex%d", i);
                if (inp.find(sampName) != std::string::npos) {
                    char decl[64];
                    FormatToA(decl, "sampler2D %s;\n", sampName);
                    chtexDecls += decl;
                }
            }
            if (!chtexDecls.empty())
        ...
        // This full-text pass handles them by scanning with FindClosingBracket across newlines.
        {
            ...
            for (int n = 2; n <= 4; n++) {
                for (int m = 2; m <= 4; m++) {
                    ...
                    {
                        char token[20]; FormatToA(token, "*%s(", mt);
                        size_t pos = 0;
    ...
    if (DLOG_DIAG_ENABLED()) {
        ...
        wchar_t diagName[64];
        FormatTo(diagName, L"diag_converter_%ls.txt", passTag);
        ...
        if (p.has(L"channels")) {
            ...
            auto parseChan = [](const JsonValue& v, int def) -> int {
                if (v.isString()) {
                    ...
                    if (s == L"self" || s == L"feedback" || Milk3PassValueIs(s, 0)) return CHAN_FEEDBACK;
                    if (Milk3PassValueIs(s, 1)) return CHAN_BUFFER_B;
                    if (Milk3PassValueIs(s, 2)) return CHAN_BUFFER_C;
                    if (Milk3PassValueIs(s, 3)) return CHAN_BUFFER_D;
                    ...
                    if (Milk3MainValueIs(s)) return CHAN_IMAGE_PREV;
