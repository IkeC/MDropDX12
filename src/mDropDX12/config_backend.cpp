...
class IniFileBackend : public ConfigBackend {
 public:
  ...
  std::vector<std::wstring> KeyNames(const wchar_t* section) const override {
    std::vector<std::wstring> names;
    if (!section) return names;
    // The whole section in one call: "key=value" runs, each null-terminated,
    // the lot ending in a second null. Reading it key by key is not possible
    // without already knowing the names, which is the thing being asked for.
    std::vector<wchar_t> buf(8192);
    DWORD n = GetPrivateProfileSectionW(section, buf.data(), (DWORD)buf.size(),
                                        m_path.c_str());
    if (n == (DWORD)buf.size() - 2) {
      buf.resize(65536);
      n = GetPrivateProfileSectionW(section, buf.data(), (DWORD)buf.size(),
                                    m_path.c_str());
    }
    for (const wchar_t* p = buf.data(); p < buf.data() + n && *p; p += wcslen(p) + 1) {
      const wchar_t* eq = wcschr(p, L'=');
      if (!eq || eq == p) continue;          // a comment or a malformed line
      names.emplace_back(p, eq);
    }
    return names;
  }
...
class RegistryBackend : public ConfigBackend {
 public:
  ...
  std::vector<std::wstring> KeyNames(const wchar_t* section) const override {
    ...
    if (!section) return names;
    HKEY hSec = nullptr;
    const std::wstring path = m_base + L"\\" + section;
    if (RegOpenKeyExW(HKEY_CURRENT_USER, path.c_str(), 0, KEY_READ, &hSec) !=
        ERROR_SUCCESS)
      return names;
    for (DWORD i = 0;; ++i) {
      wchar_t name[256];
      DWORD cch = 256;
      if (RegEnumValueW(hSec, i, name, &cch, nullptr, nullptr, nullptr,
                        nullptr) != ERROR_SUCCESS)
        break;
      names.emplace_back(name);
    }
...
bool ExportRegistryToIniFiles(std::wstring* summaryOut) {
  ...
  if (summaryOut) {
    wchar_t buf[256];
    FormatTo(buf, L"%d file(s), %d key(s)", files, keys);
