...
The annotation store lives behind one guard. Both maps and the dirty flag are
fields of `Engine::AnnotStore`, wrapped in a
`Guarded<AnnotStore, std::recursive_mutex>` (`guarded.h`, #11). They are one
unit because the hash index maps into the key map: an insert has to update both,
and two locks would need an ordering rule.

Four threads reach it -- the IPC thread, the Annotations window thread, the
Presets window thread and the render thread. Before the guard, a `clear()` or a
rehash on any one of them freed the entry another was reading through, which
killed the process with `0xC0000409` in about 260 iterations of
`test_annotations_thread_safety.py`.

There is no way to obtain a `PresetAnnotation&` except inside a
`WithAnnotation` / `WithAnnotationByHash` / `WithResolvedAnnotation` callback,
and the reference is valid for exactly the length of that callback. Each returns
`false` without calling the callback when there is no such entry -- "no entry"
and "an entry with nothing set" are different answers, and two callers depend on
the difference. Copy out what you need and act on the copy.

Four things must never happen inside a callback, and the compiler cannot catch
any of them:

- a UI `SendMessage` -- it pumps, so it re-enters, and it will deadlock against
  another thread waiting on the same guard. `LB_ADDSTRING`, `LV_InsertItemW` and
  every menu call are `SendMessage`.
- a modal dialog or a `MessageBoxW` -- it holds the lock for as long as the
  person takes to read it.
- file I/O, including `SavePresetAnnotations` and `ResolveAnnotationPath`.
- `g_pipeServer.Send`, which is blocking IPC.

Where a function has to do one of those over the whole store, it runs in phases:
snapshot under the guard, do the slow work with it released, apply under it
again. `LoadPresetAnnotations`, `SavePresetAnnotations`, `RepairAnnotationPaths`
and the `BROWSE_PRESETS` handler are all written that way.
