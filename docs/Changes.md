...
- **The Feedback Damp now applies to mirrored displays.** It shipped in v2.11.0
  covering only the primary window, so damping a preset calmed the main screen
  while every mirror kept running away. If you use mirrors, the setting now does
  what it says on all of them.

- **Song titles are positioned from their actual coordinates.** The centring
  offset was computed from an uninitialised value, so where a song title landed
  depended on whatever happened to be in memory. Positions may shift slightly
  from v2.11.0; they are now derived from the title's own X/Y.

- **The hard-cut suppression after a manual preset change now works.** Changing
  preset by hand is supposed to raise the beat threshold briefly so an automatic
  hard cut does not immediately stack on top of it. That has never actually
  happened -- the value being raised was not the one being read.
