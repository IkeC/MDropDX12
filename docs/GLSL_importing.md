...
- **[GLSL Sandbox](https://glslsandbox.com)** — simpler shaders, mostly single-pass fullscreen effects. Good for getting started.
  - **CLI import:** see [Command-line converters](#command-line-converters) below.
- **[Interactiveshaderformat.com](https://www.interactiveshaderformat.com)** — ISF-format shaders, some convertible to Shadertoy style
- **[SHADERed](https://shadered.org)** — a free shader IDE with a real debugger. Not a source of shaders so much as a place to work on one: `tools/milk3_shadered.py` converts a preset out to a SHADERed project and back.
...
- Channel values can be integers (enum index) or strings: `"self"`, `"pass0"`, `"pass1"`, `"noiseLQ"`, `"noiseMQ"`, `"noiseHQ"`, `"noiseVolLQ"`, `"noiseVolHQ"`, `"main"`, `"audio"`, `"random"` (the old `"bufferA"` / `"image"` spellings are still accepted)
...
Two Python scripts ship in `tools/`. Both are **stdlib only** — no `pip install`,
nothing to set up beyond a Python 3 that is already on the machine — and both
write plain `.milk3` files you can drop straight into `resources/presets/`.

They exist for the cases the Shader Import window is awkward for: converting a
batch of effects, scripting an import, or moving a preset out to a real shader
IDE and back.

### `glslsandbox_to_milk3.py` — fetch a GLSL Sandbox effect

[GLSL Sandbox](https://glslsandbox.com) effects are single-pass fragment
shaders. Give the script a URL or a bare id and it fetches the source and
converts it:

```
python tools/glslsandbox_to_milk3.py https://glslsandbox.com/e#109677.0
python tools/glslsandbox_to_milk3.py 109677.0
python tools/glslsandbox_to_milk3.py 109677 -o resources/presets/gs_109677.milk3
python tools/glslsandbox_to_milk3.py 109677 --keep-glsl    # embed the original GLSL too
python tools/glslsandbox_to_milk3.py 109677 --print        # write nothing, show the HLSL
```

The version suffix is part of the id and the site requires it; `.0` is added
for you if you leave it off.

Sandbox's uniforms are mapped to the milk3 equivalents:

| Sandbox | becomes |
|---|---|
| `time` | `iTime` |
| `resolution` | `iResolution` |
| `mouse` | `iMouse`, remapped to 0..1 — Sandbox's convention, and no Y flip, because milk3 stores pixels the same way up |
| `backbuffer` | `sampler_feedback`, with `channels.image.iChannel0 = "self"` |
| `surfacePosition` | derived from the pixel coordinate |

Conversion is best-effort. If the result will not compile, open it in **Shader
Import** and fix it there — the window shows the compiler's error against your
own source line.

### `milk3_shadered.py` — round-trip through SHADERed

[SHADERed](https://shadered.org) is a free shader IDE with a debugger, a
step-through and a proper editor. This converts a preset out to a SHADERed
project and back again, so you can develop in SHADERed and bring the result
home:

```
python tools/milk3_shadered.py info      preset.milk3
python tools/milk3_shadered.py info      project.zip

python tools/milk3_shadered.py to-shadered resources/presets/mine.milk3
python tools/milk3_shadered.py to-shadered preset.milk3 --zip      # one .zip instead of a folder
python tools/milk3_shadered.py to-shadered preset.milk3 --hlsl     # HLSL sources; GLSL is the default

python tools/milk3_shadered.py to-milk3   project.zip
python tools/milk3_shadered.py to-milk3   project.sprj -o out.milk3
python tools/milk3_shadered.py to-milk3   ./some_project_folder
```

`to-milk3` accepts a `.zip`, a `.sprj` or an unpacked folder, which is what you
get depending on how the project was shared.

**Multi-pass survives the trip.** A preset with Buffer A feeding Image comes
back with both passes and its channel wiring intact:

```
$ python tools/milk3_shadered.py info mine.milk3
milk3: mine.milk3
  name: mine
  passes: ['bufferA', 'image']
  channels: {'bufferA': {'iChannel0': 'self'}, 'image': {'iChannel0': 'bufferA'}}
```

**Exporting keeps your original sources.** `to-shadered` writes a `milk3_raw/`
folder alongside the shaders holding the preset's HLSL exactly as it was. If you
convert straight back without having touched anything in SHADERed, `to-milk3`
uses those rather than re-translating, so a round trip you did not mean to make
is lossless. Edit the shaders in SHADERed and it translates the edited GLSL, as
you would expect.

**GLSL out by default.** Real SHADERed projects in the wild are GLSL
(`#version 330`, `.glsl`/`.vk` files, `ScreenQuadNDC`), so that is what `to-shadered`
emits and what `to-milk3` reads most reliably. `--hlsl` exports MDropDX12's own
HLSL instead, which is closer to what the engine runs but less portable.

---

## The `.milk3` File Format

**A `.milk3` holds HLSL, not GLSL.** The conversion happens once, at import, so
nothing downstream ever sees GLSL. The stored text is plain HLSL written against
MilkDrop's own preamble — `float2`, `frac`, `time`, `texsize` — with no
`iTime`, `iResolution`, `mainImage` or `vec2` left in it.

```json
{
  "version": 1,
  "name": "my shader",
  "main":  "shader_body { ... HLSL ... }",
  "pass0": "shader_body { ... HLSL ... }",
  "inputs": {
    "pass0": { "channel0": "self" },
    "main":  { "channel0": "pass0" }
  }
...
| key | meaning |
|---|---|
| `version` | format version; identifies the file |
| `main` | the output pass — the one that reaches the screen |
| `pass0`–`pass3` | optional earlier passes, each with its own feedback buffer |
| `inputs` | which pass or texture feeds each pass's `channel0`–`channel3` |
| `name`, `notes`, `source` | metadata; `source` records where a shader came from |

### Why the keys are named this way

They used to be Shadertoy's: `image`, `bufferA`–`bufferD`, `channels`,
`iChannel0`, plus a `"shadertoy": true` flag. That was wrong twice over. The flag
was never read by anything, and it was stamped on files imported from
glslsandbox, which is a different site with a different pass model. Shadertoy's
layout is not a standard — naming the format after one site dates it and implies
a coupling that does not exist once the GLSL is gone.

**Both spellings load.** Reading accepts the old names and the new ones; writing
emits only the new ones. Every `.milk3` written by an earlier build keeps working
untouched, so there is nothing to migrate. `src/mDropDX12/milk3_format.h` is the
single place the names are defined, and the loader, the writers and the import UI
